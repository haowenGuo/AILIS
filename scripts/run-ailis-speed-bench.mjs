...
const TASKS = [
    {
        ...
        message: 'AILIS，帮我查 Playwright 官方文档，对比 locator.waitFor、expect(locator).toBeVisible、page.waitForSelector 三种等待方式。输出 playwright-waiting-comparison.md，要求说明推荐用法、timeout 设置、常见误区，每个结论都要说明来自哪个官方页面。'
    },
    {
        taskId: 'transformer_paper_code_card',
        output: 'transformer-paper-code-card.md',
        message: 'AILIS，帮我读论文 https://arxiv.org/abs/1706.03762，并查找它是否有官方或高质量复现代码。输出 transformer-paper-code-card.md，包含论文核心方法、代码仓库结构、我如果要复现最小实验应该看哪些文件。不要编造，必须区分论文证据和代码证据。'
    },
    {
        taskId: 'playwright_repo_map',
        output: 'playwright-repo-map.md',
        message: 'AILIS，帮我分析 microsoft/playwright 这个 GitHub 项目。不要全量读完，先找 README、docs、packages 结构，输出 playwright-repo-map.md。要求说明核心模块、测试入口、贡献指南、如果我要改 locator 相关功能应该从哪里开始。'
    }
];
...
async function readPreviousBaseline() {
    const filePath = path.join(LOG_DIR, 'ailis-speed-bench-2026-06-06T11-35-00-090Z.json');
    try {
...
async function main() {
    ...
    const workspaceRoot = path.join(TMP_DIR, `ailis-speed-bench-workspace-${runId}`);
    const auditDir = path.join(LOG_DIR, `ailis-speed-bench-audit-${runId}`);
    const reportPath = path.join(LOG_DIR, `ailis-speed-bench-${runId}.json`);
    const progressPath = path.join(LOG_DIR, `ailis-speed-bench-${runId}.progress.jsonl`);
