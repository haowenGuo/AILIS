...
function buildPlanForProfile(profileName, profile, options, componentManifest, outputRoot) {
    const profileOutputRoot = path.resolve(profile.outputRoot || outputRoot);
    const outputSubdir = Object.prototype.hasOwnProperty.call(profile, 'outputSubdir')
        ? String(profile.outputSubdir || '')
        : profileName;
    const outputDir = path.resolve(profileOutputRoot, outputSubdir);
    ...
    const commands = [];
    for (const command of profile.prepareCommands || []) {
        if (!command?.cmd) {
            continue;
        }
        commands.push({
            label: command.label || 'Prepare product assets',
...
async function writeReleaseManifest(plan, dryRun) {
    ...
    const manifestPath = path.join(
        plan.outputDir,
        `${plan.product}-Release-${plan.profileName}-${VERSION}.json`
    );
...
async function main() {
    ...
    const summary = {
        product: plans.length === 1 ? plans[0].product : profileConfig.product || 'AILIS',
        version: VERSION,
    ...
    if (options.json) {
        ...
        for (const plan of plans) {
            console.log(`\n[${plan.product} Release] profile=${plan.profileName}`);
    ...
    for (const plan of plans) {
        await fsp.mkdir(plan.outputDir, { recursive: true });
        for (const command of plan.commands) {
            console.log(`\n[${plan.product} Release] ${command.label}`);
            await run(command);
        }
        const manifestPath = await writeReleaseManifest(plan, false);
        console.log(`[${plan.product} Release] manifest: ${manifestPath}`);
    }
}
