...
} = require('./ailis-character-renderer-runtime.cjs');
const {
    normalizeProductCharacterRendererBackend,
    resolveProductVariant
} = require('./ailis-product-variant.cjs');
const PRODUCT_VARIANT = resolveProductVariant();
app.setName(PRODUCT_VARIANT.productName);
const {
    ...
    DEFAULT_CHARACTER_RENDERER_BACKEND,
    DEFAULT_UNITY_CHARACTER_PACKAGE_ID,
    ...
    DEFAULT_UNITY_RENDERER_SETTINGS,
    ...
    normalizeCharacterRendererBackend,
    normalizeUnityCharacterPackageId,
    ...
    normalizeUnityRendererSettings,
...
const CHARACTER_LAB_MIN_WIDTH = 820;
const CHARACTER_LAB_MIN_HEIGHT = 620;
const DIALOGUE_SURFACE_WIDTH = 540;
const DIALOGUE_SURFACE_HEIGHT = 230;
const DIALOGUE_SURFACE_GAP = 12;

function normalizeAvailableCharacterRendererBackend(value) {
    return normalizeProductCharacterRendererBackend(value, PRODUCT_VARIANT);
}
...
const MENU_I18N = Object.freeze({
    en: Object.freeze({
        ...
        characterLab: 'Character Test Lab',
        chat: 'Chat',
        language: 'Language',
...
const PET_DRAG_SYNC_HZ = 60;
const PET_DRAG_DIALOGUE_INTERVAL_MS = 34;
const CHARACTER_HIT_TEST_DEBUG = process.env.AILIS_CHARACTER_HIT_TEST_DEBUG === '1';
...
let characterLabWindow = null;
let characterLabWindowLoadPromise = null;
let tray = null;
...
let characterRendererRuntime = null;
let unityPetPlacementInitialized = false;
...
let petInteractionRaiseTimer = null;
...
function getUnityPetVisibleContentBounds() {
    const rendererStatus = characterRendererRuntime?.getStatus?.();
    const hitTestBounds = rendererStatus?.hitTestBounds;
    if (
        normalizeCharacterRendererBackend(
            desktopState?.preferences?.characterRendererBackend
        ) !== 'unity' ||
        rendererStatus?.status !== 'ready' ||
        !hitTestBounds ||
        hitTestBounds.complete === false
    ) {
        return null;
    }
...
function getPetVisibleContentScreenBounds(bounds, contentBounds = getUnityPetVisibleContentBounds()) {
    if (!bounds || !contentBounds) {
        return null;
    }
    return {
        x: Math.round(Number(bounds.x) + Number(contentBounds.left)),
        y: Math.round(Number(bounds.y) + Number(contentBounds.top)),
        width: Math.max(1, Math.round(Number(contentBounds.width))),
        height: Math.max(1, Math.round(Number(contentBounds.height)))
    };
}

function constrainPetBoundsToVisibleDisplay(bounds, { forceWholeWindow = false } = {}) {
    const resizedBounds = resizePetBounds(bounds, getCurrentPetScale());
    const contentBounds = forceWholeWindow ? null : getUnityPetVisibleContentBounds();
    if (contentBounds) {
        const contentScreenBounds = getPetVisibleContentScreenBounds(resizedBounds, contentBounds);
        const display = screen.getDisplayMatching(contentScreenBounds);
        const contentConstrainedBounds = clampRendererWindowByVisibleContent(
            resizedBounds,
            contentBounds,
            display.workArea
        );
        if (contentConstrainedBounds) {
            return contentConstrainedBounds;
        }
    }

    const unitySelected = normalizeCharacterRendererBackend(
        desktopState?.preferences?.characterRendererBackend
    ) === 'unity';
    const rendererStatus = characterRendererRuntime?.getStatus?.().status;
    if (!forceWholeWindow && unitySelected && rendererStatus !== 'fallback') {
        // Before Unity publishes its first pixel bounds, preserve the saved
        // canvas offset. The first bounds event will constrain the avatar itself.
...
function petBoundsEqual(left, right) {
    return Boolean(
        left &&
        right &&
        left.x === right.x &&
...
function applyWindowBoundsEfficiently(window, bounds) {
...
function applyPetDragFrame(payload = {}) {
    if (!petWindow || petWindow.isDestroyed() || !petDragState) {
        return false;
    }

    const cursor = screen.getCursorScreenPoint();
    const rawDeltaX = Number(payload.deltaX);
    const rawDeltaY = Number(payload.deltaY);
    const deltaX = petDragState.cursor && petDragState.baseBounds
        ? cursor.x - petDragState.cursor.x
        : Number.isFinite(rawDeltaX) ? rawDeltaX : 0;
    const deltaY = petDragState.cursor && petDragState.baseBounds
        ? cursor.y - petDragState.cursor.y
        : Number.isFinite(rawDeltaY) ? rawDeltaY : 0;
    const bounds = petDragState.baseBounds || petWindow.getBounds();
    const nextBounds = constrainPetBoundsToVisibleDisplay({
        ...bounds,
        x: Math.round(bounds.x + deltaX),
        y: Math.round(bounds.y + deltaY)
    });

    if (petBoundsEqual(petDragState.lastAppliedBounds, nextBounds)) {
        return false;
    }
    petDragState.lastAppliedBounds = { ...nextBounds };
    applyWindowBoundsEfficiently(petWindow, nextBounds);
    desktopState.petWindow.bounds = nextBounds;
    desktopState.petWindow.visible = petWindow.isVisible();

    const now = Date.now();
    if (now - petDragState.lastDialogueUpdateAt >= PET_DRAG_DIALOGUE_INTERVAL_MS) {
        petDragState.lastDialogueUpdateAt = now;
        updateDialogueSurfacePosition();
    }
    return true;
}

function startPetDragPump() {
    stopPetDragPump();
    if (!petDragState?.baseBounds) {
        return false;
    }
    petDragTimer = setInterval(() => {
        applyPetDragFrame();
    }, Math.round(1000 / PET_DRAG_SYNC_HZ));
    petDragTimer.unref?.();
    return true;
}
...
function finishPetDrag() {
    ...
    if (petWindow && !petWindow.isDestroyed()) {
        updateWindowState('petWindow', petWindow, {
            rendererWindowPhase: 'settle'
        });
        schedulePetInteractionSurfaceRaise(80);
    }
    return true;
}

function restorePetWindowToVisibleDisplay({ persist = false } = {}) {
    const currentBounds = petWindow && !petWindow.isDestroyed()
        ? petWindow.getBounds()
        : desktopState?.petWindow?.bounds;
    if (!currentBounds || !desktopState?.petWindow) {
        return null;
    }

    const safeBounds = constrainPetBoundsToVisibleDisplay(currentBounds);
    const changed = !petBoundsEqual(currentBounds, safeBounds) ||
        !petBoundsEqual(desktopState.petWindow.bounds, safeBounds);
    desktopState.petWindow.bounds = safeBounds;
    if (petWindow && !petWindow.isDestroyed() && !petBoundsEqual(petWindow.getBounds(), safeBounds)) {
...
function setPetMousePassthrough(enabled, options = {}) {
    ...
    if (CHARACTER_HIT_TEST_DEBUG) {
        console.log('[character-hit-test] passthrough', nextEnabled);
    }
...
function startPetCursorTracking() {
    ...
    petCursorTrackingTimer = setInterval(() => {
        ...
        const rendererStatus = characterRendererRuntime?.getStatus?.();
        const useUnityHitTest = rendererStatus?.status === 'ready' &&
            normalizeCharacterRendererBackend(
                desktopState?.preferences?.characterRendererBackend
            ) === 'unity';
        if (useUnityHitTest && !petDragState) {
            const hitAvatar = inside && pointInRendererHitTestBounds({
                x: clientX,
                y: clientY
            }, rendererStatus.hitTestBounds);
            setPetMousePassthrough(!hitAvatar);
        }
...
function getPersistedElevenLabsSettings(payload = {}) {
    ...
    return {
        ...
        outputFormat: normalizeElevenLabsOutputFormat(
            payload.outputFormat || payload.output_format ||
                selectedProfile.outputFormat ||
                preferences.elevenLabsOutputFormat ||
                DEFAULT_ELEVENLABS_OUTPUT_FORMAT
        ),
...
function ensureAILISGateway() {
    ...
    ailisGateway = new AILISGateway({
        ...
        voiceServices: {
            tts: (payload) => callDesktopTts(payload || {}),
            asr: async (audioBytes) => {
                if (!desktopASRManager) {
                    throw new Error('本地语音识别管理器尚未初始化');
                }
                return await desktopASRManager.transcribeAudioBytes(audioBytes);
            }
        },
...
function getRendererPreferences() {
    return {
        ...
        characterRendererBackend: normalizeAvailableCharacterRendererBackend(
            desktopState?.preferences?.characterRendererBackend || DEFAULT_CHARACTER_RENDERER_BACKEND
        ),
        unityCharacterPackageId: normalizeUnityCharacterPackageId(
            desktopState?.preferences?.unityCharacterPackageId || DEFAULT_UNITY_CHARACTER_PACKAGE_ID
        ),
        characterRendererRuntime: characterRendererRuntime?.getStatus?.() || {
            desiredBackend: normalizeAvailableCharacterRendererBackend(
                desktopState?.preferences?.characterRendererBackend || DEFAULT_CHARACTER_RENDERER_BACKEND
            ),
        ...
        unityRenderer: normalizeUnityRendererSettings(
            desktopState?.preferences?.unityRenderer || DEFAULT_UNITY_RENDERER_SETTINGS
        ),
...
function getControlPanelState() {
    return {
        ...
        options: {
            ...
            characterRendererBackendOptions: [...PRODUCT_VARIANT.characterRendererBackends],
        ...
        environment: {
            ...
            productName: PRODUCT_VARIANT.productName,
            productFeatures: PRODUCT_VARIANT.features,
            userDataPath: app.getPath('userData'),
            projectRoot: getProjectRoot()
        }
    };
...
function broadcastPreferencesUpdated() {
    ...
    personaHostWindow?.webContents.send('ailis:preferences-updated', payload);
    dialogueWindow?.webContents.send('ailis:preferences-updated', payload);
    controlWindow?.webContents.send('ailis:preferences-updated', payload);
}

function schedulePetInteractionSurfaceRaise(delayMs = 0) {
    if (petInteractionRaiseTimer) {
        clearTimeout(petInteractionRaiseTimer);
    }
    petInteractionRaiseTimer = setTimeout(() => {
        petInteractionRaiseTimer = null;
        const useUnity = characterRendererRuntime?.getStatus?.().status === 'ready' &&
            normalizeCharacterRendererBackend(
                desktopState?.preferences?.characterRendererBackend
            ) === 'unity';
        if (!useUnity || !petWindow || petWindow.isDestroyed()) {
            return;
        }
        petWindow.setAlwaysOnTop(true, 'screen-saver');
        petWindow.moveTop();
    }, Math.max(0, Number(delayMs) || 0));
    petInteractionRaiseTimer.unref?.();
}
...
function updateWindowState(key, window, options = {}) {
    ...
    desktopState[key].bounds = key === 'petWindow'
        ? constrainPetBoundsToVisibleDisplay(window.getBounds())
    ...
    if (key === 'petWindow') {
        ...
        syncCharacterRendererWindowBounds({
            phase: options.rendererWindowPhase
        });
        updateDialogueSurfacePosition();
    }
...
const RENDERER_LOAD_RETRY_DELAYS_MS = [0, 120, 300, 700, 1400];

function waitForRendererLoadRetry(delayMs) {
    return new Promise((resolve) => {
...
async function loadWindowContent(window, pageName) {
    if (isDevMode()) {
        return window.loadURL(buildRendererUrl(pageName));
    }

    const rendererPath = buildRendererUrl(pageName);
    let lastError = null;
    for (let attempt = 0; attempt < RENDERER_LOAD_RETRY_DELAYS_MS.length; attempt += 1) {
        const retryDelayMs = RENDERER_LOAD_RETRY_DELAYS_MS[attempt];
        if (retryDelayMs > 0) {
            await waitForRendererLoadRetry(retryDelayMs);
        }
        if (!window || window.isDestroyed() || window.webContents?.isDestroyed()) {
            throw new Error(`Renderer window was destroyed before ${pageName} could load.`);
        }
        if (!fs.existsSync(rendererPath)) {
            lastError = new Error(`Renderer page is not ready: ${rendererPath}`);
            continue;
        }
        try {
            ...
            lastError = error;
            console.warn(
                `[renderer:${pageName}] load attempt ${attempt + 1}/${RENDERER_LOAD_RETRY_DELAYS_MS.length} failed:`,
                error?.message || error
            );
        }
    }
    throw lastError || new Error(`Renderer page failed to load: ${rendererPath}`);
}
...
function getDialogueSurfaceBounds() {
    const characterBounds = desktopState?.petWindow?.bounds;
    if (!characterBounds) {
        return null;
    }
    const anchorBounds =
        getPetVisibleContentScreenBounds(characterBounds) ||
        characterBounds;
    const display = screen.getDisplayMatching(anchorBounds);
    const workArea = display.workArea;
    const width = Math.min(DIALOGUE_SURFACE_WIDTH, workArea.width);
    const height = Math.min(DIALOGUE_SURFACE_HEIGHT, workArea.height);
    const centeredX = Math.round(anchorBounds.x + anchorBounds.width / 2 - width / 2);
    let x = Math.min(Math.max(centeredX, workArea.x), workArea.x + workArea.width - width);
    let y = anchorBounds.y - height - DIALOGUE_SURFACE_GAP;

    if (y < workArea.y) {
        const rightX = anchorBounds.x + anchorBounds.width + DIALOGUE_SURFACE_GAP;
        const leftX = anchorBounds.x - width - DIALOGUE_SURFACE_GAP;
        ...
        y = Math.min(
            Math.max(anchorBounds.y, workArea.y),
...
function updateDialogueSurfacePosition({ force = false } = {}) {
    if (!dialogueWindow || dialogueWindow.isDestroyed()) {
        return false;
    }
    if (!force && !dialogueWindow.isVisible()) {
        return false;
    }
    const bounds = getDialogueSurfaceBounds();
    if (!bounds) {
        return false;
    }
    return applyWindowBoundsEfficiently(dialogueWindow, bounds);
}

function hideDialogueSurface() {
    clearTimeout(dialogueHideTimer);
    dialogueHideTimer = null;
    if (dialogueWindow && !dialogueWindow.isDestroyed()) {
        dialogueWindow.hide();
    }
}

function publishDialogueSurfaceEvent(payload = {}) {
    if (desktopState?.petWindow?.visible === false) {
        return false;
    }
    if (!dialogueWindow || dialogueWindow.isDestroyed()) {
    ...
    clearTimeout(dialogueHideTimer);
    dialogueHideTimer = null;
    updateDialogueSurfacePosition({ force: true });
    dialogueWindow.showInactive();
    desktopPlatformAdapter.setMousePassthrough(dialogueWindow, true, {
        forward: true
    });
    schedulePetInteractionSurfaceRaise(0);
    dialogueWindow.webContents.send('ailis:dialogue-surface-event', payload);

    const phase = String(payload.phase || payload.type || '').toLowerCase();
    if (phase === 'end') {
        dialogueHideTimer = setTimeout(hideDialogueSurface, 2400);
        dialogueHideTimer.unref?.();
    }
    return true;
}
...
function showCharacterLabWindow() {
    if (!characterLabWindow) {
        createCharacterLabWindow({ showWhenReady: true });
        return true;
    }

    characterLabWindow.__ailisShowWhenReady = true;
    const isLoaded = Boolean(characterLabWindow.__ailisDidFinishLoad);
    if (!characterLabWindow.isVisible() && isLoaded) {
        characterLabWindow.show();
    }

    if (isLoaded) {
        characterLabWindow.focus();
    } else {
        characterLabWindowLoadPromise?.then(() => {
            if (!characterLabWindow || characterLabWindow.isDestroyed()) {
                return;
            }
            if (characterLabWindow.__ailisShowWhenReady && !characterLabWindow.isVisible()) {
                characterLabWindow.show();
            }
            characterLabWindow.focus();
        }).catch((error) => {
            console.error('[window] 角色功能测试台延迟显示失败：', error);
        });
    }
    return true;
}
...
function applyPreferencesPatch(partialPreferences = {}) {
    ...
    const nextPreferences = {
        ...
        characterRendererBackend: rendererPreferences.characterRendererBackend,
        unityCharacterPackageId: rendererPreferences.unityCharacterPackageId,
        ...
        unityRenderer: rendererPreferences.unityRenderer,
    ...
    if ('characterRendererBackend' in partialPreferences) {
        nextPreferences.characterRendererBackend = normalizeAvailableCharacterRendererBackend(
            partialPreferences.characterRendererBackend
        );
    }
    if ('unityCharacterPackageId' in partialPreferences) {
        nextPreferences.unityCharacterPackageId = normalizeUnityCharacterPackageId(
            partialPreferences.unityCharacterPackageId
        );
    }
    ...
    if ('unityRenderer' in partialPreferences) {
        nextPreferences.unityRenderer = normalizeUnityRendererSettings(partialPreferences.unityRenderer);
    }
    ...
    const characterRendererBackendChanged =
        nextPreferences.characterRendererBackend !== rendererPreferences.characterRendererBackend;
    const unityCharacterPackageChanged =
        nextPreferences.unityCharacterPackageId !== rendererPreferences.unityCharacterPackageId;
    ...
    if (petScaleChanged) {
        const referenceBounds = petWindow
            ? petWindow.getBounds()
            : desktopState.petWindow.bounds;
        const nextBounds = constrainPetBoundsToVisibleDisplay(
    ...
    const unityRendererSettingsChanged = 'unityRenderer' in partialPreferences;

    if (characterRendererBackendChanged || unityCharacterPackageChanged || petScaleChanged) {
        void syncCharacterRenderer('preferences_changed');
    } else if (
        nextPreferences.characterRendererBackend === 'unity' &&
        characterRendererRuntime?.getStatus?.().status === 'ready' &&
        unityRendererSettingsChanged
    ) {
        characterRendererRuntime.sendConfiguration(
            buildUnityRendererConfiguration()
        );
    }
...
function buildControlMenuTemplate({ includeTaskbarToggle = false } = {}) {
    const template = [
        ...
        {
            label: menuText('characterLab'),
            click: () => showCharacterLabWindow()
        },
        {
            label: menuText('chat'),
...
function showControlMenu(targetWindow = petWindow, screenPoint = null) {
    const hasTargetWindow = Boolean(targetWindow && !targetWindow.isDestroyed());
    const hasScreenPoint = Number.isFinite(Number(screenPoint?.x)) &&
        Number.isFinite(Number(screenPoint?.y));
    if (!hasTargetWindow && !hasScreenPoint) {
        return false;
    }
    ...
    if (hasTargetWindow) {
        ...
        targetWindow.focus();
        popupOptions.window = targetWindow;
    }
    if (hasScreenPoint) {
        popupOptions.x = Math.round(Number(screenPoint.x));
        popupOptions.y = Math.round(Number(screenPoint.y));
    }
    if (CHARACTER_HIT_TEST_DEBUG) {
        console.log('[character-hit-test] context menu popup', {
            attachedToPetWindow: popupOptions.window === petWindow,
            hasScreenPoint
        });
    }
    buildPetContextMenu().popup(popupOptions);
    return true;
}
...
function ensureCharacterRendererRuntime() {
    if (characterRendererRuntime) {
        return characterRendererRuntime;
    }
    characterRendererRuntime = new AILISCharacterRendererRuntime({
        enabled: PRODUCT_VARIANT.features.unityCharacterRenderer,
        projectRoot: getProjectRoot(),
        resourcesPath: process.resourcesPath,
        platform: process.platform
    });
    characterRendererRuntime.on('ready', () => {
        unityPetPlacementInitialized = false;
        applyCharacterRendererPresentation('unity');
        broadcastPreferencesUpdated();
    });
    characterRendererRuntime.on('hit-test-bounds', (hitTestBounds) => {
        if (CHARACTER_HIT_TEST_DEBUG) {
            console.log('[character-hit-test] Unity bounds', hitTestBounds);
        }
        const useUnity = characterRendererRuntime?.getStatus?.().status === 'ready' &&
            normalizeCharacterRendererBackend(
                desktopState?.preferences?.characterRendererBackend
            ) === 'unity' &&
            desktopState?.petWindow?.visible !== false;
        if (useUnity) {
            if (!unityPetPlacementInitialized && hitTestBounds) {
                unityPetPlacementInitialized = true;
                restorePetWindowToVisibleDisplay({ persist: true });
                syncCharacterRendererWindowBounds();
                updateDialogueSurfacePosition();
            }
            publishPetRendererInteractionState('unity', hitTestBounds);
        }
    });
    characterRendererRuntime.on('event', (event = {}) => {
        if (event.type === 'renderer.window.drag_released') {
            finishPetDrag();
            return;
        }
        if (event.type === 'renderer.window.settled') {
            schedulePetInteractionSurfaceRaise(0);
        }
    });
    characterRendererRuntime.on('fallback', (status) => {
        unityPetPlacementInitialized = false;
        console.warn('[character-renderer] Unity 回退到 Electron：', status.error || status.status);
        applyCharacterRendererPresentation('electron');
        broadcastPreferencesUpdated();
    });
    return characterRendererRuntime;
}

function buildUnityRendererConfiguration() {
    const runtime = ensureCharacterRendererRuntime();
    return runtime.createConfiguration({
        preferences: getRendererPreferences()
    });
}

function getCharacterRendererWindowGeometry(bounds) {
    try {
        const display = screen.getDisplayMatching(
            getPetVisibleContentScreenBounds(bounds) || bounds
        );
        return {
            bounds: {
                ...bounds,
            ...
            displayScaleFactor: Math.max(1, Number(display.scaleFactor || 1))
        };
    } catch {
        return { bounds, displayScaleFactor: 1 };
    }
}

function syncCharacterRendererWindowBounds(options = {}) {
    if (
        characterRendererRuntime?.getStatus?.().status !== 'ready' ||
        normalizeCharacterRendererBackend(desktopState?.preferences?.characterRendererBackend) !== 'unity'
    ) {
        return false;
    }
    const bounds = options.bounds ||
        restorePetWindowToVisibleDisplay() ||
        desktopState?.petWindow?.bounds;
    if (!bounds) {
        return false;
    }
    const geometry = getCharacterRendererWindowGeometry(bounds);
    const sent = characterRendererRuntime.sendWindowBounds(geometry.bounds, {
        displayScaleFactor: geometry.displayScaleFactor,
        phase: options.phase
    });
    if (sent && !petDragState) {
        schedulePetInteractionSurfaceRaise(250);
    }
    return sent;
}

function publishPetRendererInteractionState(backend, hitTestBounds = null) {
    if (!petWindow || petWindow.isDestroyed() || petWindow.webContents.isDestroyed()) {
        return false;
    }
    petWindow.webContents.send('ailis:pet-renderer-state', {
        backend: backend === 'unity' ? 'unity' : 'electron',
        hitTestBounds: hitTestBounds || null
    });
    return true;
}

function applyCharacterRendererPresentation(backend) {
    if (!petWindow || petWindow.isDestroyed()) {
        return;
    }
    const safeBounds = restorePetWindowToVisibleDisplay();
    if (safeBounds && !petBoundsEqual(petWindow.getBounds(), safeBounds)) {
        petWindow.setBounds(safeBounds);
    }
    const useUnity = backend === 'unity' && desktopState?.petWindow?.visible !== false;
    if (useUnity) {
        // Keep the transparent Electron surface alive for hit testing. The pet
        // renderer hides only its Three.js canvas while Unity owns presentation.
        petWindow.setOpacity(1);
        setPetMousePassthrough(true, { force: true });
        startPetCursorTracking();
        schedulePetInteractionSurfaceRaise(80);
        publishPetRendererInteractionState(
            'unity',
            characterRendererRuntime?.getStatus?.().hitTestBounds || null
        );
        return;
    }
    petWindow.setOpacity(1);
    const electronBounds = safeBounds || desktopState?.petWindow?.bounds;
    if (electronBounds) {
        const unitySelected = normalizeCharacterRendererBackend(
            desktopState?.preferences?.characterRendererBackend
        ) === 'unity';
        petWindow.setBounds(
            unitySelected
                ? constrainPetBoundsToVisibleDisplay(electronBounds)
                : canonicalizePetBounds(electronBounds)
        );
    }
    setPetMousePassthrough(true, { force: true });
    startPetCursorTracking();
    publishPetRendererInteractionState('electron');
}

async function syncCharacterRenderer(reason = 'sync') {
    const runtime = ensureCharacterRendererRuntime();
    const desiredBackend = normalizeAvailableCharacterRendererBackend(
        desktopState?.preferences?.characterRendererBackend || PRODUCT_VARIANT.defaultCharacterRendererBackend
    );
    if (desktopState?.petWindow?.visible === false) {
        await runtime.stop('pet_hidden');
        applyCharacterRendererPresentation('electron');
        return runtime.getStatus();
    }
    if (desiredBackend === 'electron') {
        unityPetPlacementInitialized = false;
    ...
    // Keep the Three canvas hidden while Unity starts or switches packages.
    // Fallback events explicitly restore Electron if Unity cannot become ready.
    applyCharacterRendererPresentation('unity');
    unityPetPlacementInitialized = false;
    const safeBounds = restorePetWindowToVisibleDisplay({ persist: true }) || desktopState.petWindow.bounds;
    const geometry = getCharacterRendererWindowGeometry(safeBounds);
    const status = await runtime.activate('unity', {
        bounds: geometry.bounds,
        displayScaleFactor: geometry.displayScaleFactor,
        preferences: getRendererPreferences(),
        characterPackageId: normalizeUnityCharacterPackageId(
            desktopState?.preferences?.unityCharacterPackageId
        ),
        reason
    });
...
function sendCharacterRendererCommand(command) {
    if (characterRendererRuntime?.getStatus?.().status !== 'ready') {
        return false;
    }
    return characterRendererRuntime.send(command);
}

function createPersonaHostWindow() {
    if (personaHostWindow && !personaHostWindow.isDestroyed()) {
        return personaHostWindow;
    }
    personaHostWindow = desktopPlatformAdapter.createWindow({
        ...
        alwaysOnTop: false,
        show: false,
        title: 'AILIS Persona Host',
        webPreferences: {
            backgroundThrottling: false
        }
    });
    hookRendererDiagnostics(personaHostWindow, 'persona-host');
    personaHostWindow.on('close', (event) => {
        if (!isQuitting) {
            event.preventDefault();
        }
    });
    personaHostWindow.on('closed', () => {
        personaHostWindow = null;
    });
    void loadWindowContent(personaHostWindow, 'persona-host.html').catch((error) => {
        console.error('[window] Persona Host 加载失败：', error);
    });
    return personaHostWindow;
}
...
function createDialogueWindow() {
    ...
    dialogueWindow = desktopPlatformAdapter.createWindow({
        bounds: getDialogueSurfaceBounds() || {
            x: 0,
            y: 0,
            width: DIALOGUE_SURFACE_WIDTH,
            height: DIALOGUE_SURFACE_HEIGHT
        },
        frame: false,
        transparent: true,
        ...
        alwaysOnTop: true,
        show: false,
        title: 'AILIS Dialogue Surface'
    });
    desktopPlatformAdapter.applyWindowBehavior(dialogueWindow, {
        alwaysOnTop: true,
        alwaysOnTopLevel: 'screen-saver',
        visibleOnAllWorkspaces: true,
        visibleOnFullScreen: true,
        mousePassthrough: true,
        forwardMouseMessages: true
    });
    hookRendererDiagnostics(dialogueWindow, 'dialogue-surface');
    dialogueWindow.on('close', (event) => {
        if (!isQuitting) {
            event.preventDefault();
            dialogueWindow.hide();
        }
    ...
    void loadWindowContent(dialogueWindow, 'dialogue.html').catch((error) => {
        console.error('[window] Dialogue Surface 加载失败：', error);
    });
    return dialogueWindow;
}
...
function createPetWindow() {
    ...
    const petBounds = constrainPetBoundsToVisibleDisplay(petState.bounds);
    ...
    petWindow.on('close', (event) => {
        ...
        event.preventDefault();
        desktopState.petWindow.visible = false;
        void characterRendererRuntime?.stop?.('pet_window_closed');
        hideDialogueSurface();
        petWindow.hide();
        hideChatWindow();
    });
    ...
    void loadWindowContent(loadingPetWindow, 'pet.html')
        .then(async () => {
            if (
                loadingPetWindow !== petWindow ||
                loadingPetWindow.isDestroyed() ||
                loadingPetWindow.webContents.isDestroyed()
            ) {
                return;
            }
            setPetMousePassthrough(true, { force: true });
            ...
            if (!desktopState.petWindow.visible) {
                loadingPetWindow.hide();
            }
            await syncCharacterRenderer('pet_window_ready');
        })
        .catch((error) => {
            console.error('[window] 桌宠窗口加载失败，Unity 渲染器不会脱离交互层单独启动：', error);
            void characterRendererRuntime?.stop?.('pet_window_load_failed');
        });
}
...
function createChatWindow() {
    ...
    desktopPlatformAdapter.applyWindowBehavior(chatWindow, {
        alwaysOnTop: true,
        alwaysOnTopLevel: 'screen-saver'
    });
...
function createCharacterLabWindow(options = {}) {
    const display = screen.getPrimaryDisplay();
    const workArea = display.workArea;
    const width = Math.min(980, Math.max(CHARACTER_LAB_MIN_WIDTH, workArea.width - 72));
    const height = Math.min(760, Math.max(CHARACTER_LAB_MIN_HEIGHT, workArea.height - 72));
    const bounds = clampBoundsToDisplay(
        {
            x: Math.round(workArea.x + (workArea.width - width) / 2),
            y: Math.round(workArea.y + (workArea.height - height) / 2),
        ...
        CHARACTER_LAB_MIN_WIDTH,
        CHARACTER_LAB_MIN_HEIGHT
    );
    const showWhenReady = Boolean(options.showWhenReady);

    characterLabWindow = desktopPlatformAdapter.createWindow({
        bounds,
        minWidth: CHARACTER_LAB_MIN_WIDTH,
        minHeight: CHARACTER_LAB_MIN_HEIGHT,
        frame: false,
        transparent: false,
        ...
        show: false,
        skipTaskbar: false,
        title: 'AILIS Character Test Lab'
    });
    characterLabWindow.__ailisDidFinishLoad = false;
    characterLabWindow.__ailisShowWhenReady = showWhenReady;

    openExternalLinks(characterLabWindow);
    hookRendererDiagnostics(characterLabWindow, 'character-lab');
    hookWindowContextMenu(characterLabWindow, 'character-lab');

    characterLabWindow.on('close', (event) => {
        if (isQuitting) {
            return;
        }
        event.preventDefault();
        hideDialogueSurface();
        characterLabWindow.hide();
    });

    characterLabWindow.on('closed', () => {
        characterLabWindow = null;
        characterLabWindowLoadPromise = null;
    });

    characterLabWindowLoadPromise = loadWindowContent(characterLabWindow, 'character-lab.html')
        .then(() => {
            if (!characterLabWindow || characterLabWindow.isDestroyed()) {
                return;
            }
            characterLabWindow.__ailisDidFinishLoad = true;
            if (characterLabWindow.__ailisShowWhenReady) {
                characterLabWindow.show();
                characterLabWindow.focus();
            }
        })
        .catch((error) => {
            console.error('[window] 角色功能测试台加载失败：', error);
        });
}
...
function refreshTrayMenu() {
    ...
    const menu = Menu.buildFromTemplate([
        {
            label: desktopState?.petWindow?.visible !== false ? menuText('hidePet') : menuText('showPet'),
            click: () => {
                if (!petWindow) {
                    desktopState.petWindow.visible = true;
                    createPetWindow();
                    persistDesktopState();
                    return;
                }
                if (desktopState.petWindow.visible !== false) {
                    desktopState.petWindow.visible = false;
                    void characterRendererRuntime?.stop?.('pet_hidden_from_tray');
                    hideDialogueSurface();
                    petWindow.hide();
                    hideChatWindow();
                } else {
                    ...
                    petWindow.show();
                    petWindow.focus();
                    void syncCharacterRenderer('pet_shown_from_tray');
                }
                persistDesktopState();
            }
        },
...
function createTray() {
    ...
    tray.on('double-click', () => {
        ...
        petWindow.focus();
        persistDesktopState();
        void syncCharacterRenderer('pet_shown_from_tray_double_click');
    });
    refreshTrayMenu();
}
...
function registerIpc() {
    ...
    ipcMain.handle('ailis:show-character-lab', () => showCharacterLabWindow());
    ipcMain.handle('ailis:get-character-renderer-status', () => (
        ensureCharacterRendererRuntime().getStatus()
    ));
    ipcMain.handle('ailis:get-character-lab-capabilities', (event) => {
        const sourceWindow = BrowserWindow.fromWebContents(event.sender);
        if (!sourceWindow || sourceWindow !== characterLabWindow) {
            return {
                schema: 'ailis.character-capabilities.v1',
                backend: 'unknown',
                expressions: [],
                motions: [],
                error: 'character_lab_sender_not_allowed'
            };
        }
        return ensureCharacterRendererRuntime().getCapabilities();
    });
    ipcMain.handle('ailis:character-lab-select-character', async (event, payload = {}) => {
        const sourceWindow = BrowserWindow.fromWebContents(event.sender);
        if (!sourceWindow || sourceWindow !== characterLabWindow) {
            return { ok: false, error: 'character_lab_sender_not_allowed' };
        }
        const packageId = normalizeUnityCharacterPackageId(payload.packageId);
        const runtime = ensureCharacterRendererRuntime();
        const selected = runtime.listCharacterPackages().find(
            (item) => item.id === packageId
        );
        if (!selected) {
            return { ok: false, error: 'character_package_not_installed' };
        }
        desktopState.preferences.unityCharacterPackageId = selected.id;
        persistDesktopState();
        broadcastPreferencesUpdated();
        const status = await syncCharacterRenderer('character_lab_package_changed');
        return {
            ok: status.status !== 'fallback',
            status,
            capabilities: runtime.getCapabilities(),
            error: status.status === 'fallback' ? status.error : ''
        };
    });
    ipcMain.handle('ailis:character-lab-apply-surface', (event, payload = {}) => {
        const sourceWindow = BrowserWindow.fromWebContents(event.sender);
        if (!sourceWindow || sourceWindow !== characterLabWindow) {
            return { ok: false, error: 'character_lab_sender_not_allowed' };
        }
        const command = {
            ...
            requestId: String(payload.requestId || `character-lab-${Date.now()}`),
            surface: payload.surface && typeof payload.surface === 'object'
                ? payload.surface
                : {}
        };
        const useUnity = characterRendererRuntime?.getStatus?.().status === 'ready' &&
        ...
        if (useUnity) {
            sendCharacterRendererCommand(command);
        } else {
            petWindow?.webContents.send('ailis:character-renderer-command', command);
        }
        return {
            ok: true,
            backend: useUnity ? 'unity' : 'electron',
            requestId: command.requestId
        };
    });
    ipcMain.handle('ailis:character-lab-play-motion', (event, payload = {}) => {
        const sourceWindow = BrowserWindow.fromWebContents(event.sender);
        if (!sourceWindow || sourceWindow !== characterLabWindow) {
            return { ok: false, error: 'character_lab_sender_not_allowed' };
        }
        const motionId = String(payload.motionId || '').trim();
        if (!motionId || motionId.length > 120) {
            return { ok: false, error: 'invalid_motion_id' };
        }
        const runtime = ensureCharacterRendererRuntime();
        const capability = runtime.getCapabilities()?.motions?.find(
            (motion) => String(motion?.id || '').toLowerCase() === motionId.toLowerCase()
        );
        if (!capability) {
            return { ok: false, error: 'motion_not_in_active_character_package' };
        }
        const useUnity = runtime.getStatus?.().status === 'ready' &&
            normalizeCharacterRendererBackend(
                desktopState?.preferences?.characterRendererBackend
            ) === 'unity';
        if (!useUnity) {
            return { ok: false, error: 'unity_character_renderer_not_ready' };
        }
        const command = {
            type: 'character.action',
            requestId: String(payload.requestId || `character-lab-motion-${Date.now()}`),
            action: { motionId: capability.id }
        };
        const sent = sendCharacterRendererCommand(command);
        return {
            ok: sent,
            backend: 'unity',
            requestId: command.requestId,
            motionId: capability.id,
            error: sent ? '' : 'character_renderer_command_not_sent'
        };
    });
    ipcMain.handle('ailis:character-lab-animation-state', async (event) => {
        const sourceWindow = BrowserWindow.fromWebContents(event.sender);
        if (!sourceWindow || sourceWindow !== characterLabWindow) {
            return {
                ok: false,
        ...
        return ensureCharacterRendererRuntime()
            .requestAnimationDebugState();
    });
    ipcMain.handle(
        ...
        async (event, payload = {}) => {
            ...
            return ensureCharacterRendererRuntime()
                .controlAnimationDebug(payload || {});
        }
    );
    ipcMain.handle('ailis:character-lab-publish-bubble', (event, payload = {}) => {
        const sourceWindow = BrowserWindow.fromWebContents(event.sender);
        if (!sourceWindow || sourceWindow !== characterLabWindow) {
            return { ok: false, error: 'character_lab_sender_not_allowed' };
        }
        return { ok: publishDialogueSurfaceEvent(payload) };
    });
    ipcMain.handle('ailis:character-lab-hide-bubble', (event) => {
        const sourceWindow = BrowserWindow.fromWebContents(event.sender);
        if (!sourceWindow || sourceWindow !== characterLabWindow) {
            return { ok: false, error: 'character_lab_sender_not_allowed' };
        }
    ...
    ipcMain.handle('ailis:memory-strategies', async () => {
        const gateway = ensureAILISGateway();
        const memoryStatus = gateway.memoryRuntime?.getStatus?.() || {};
        return {
            ok: true,
            active: memoryStatus.memoryStrategy || '',
            strategies: gateway.getMemoryStrategyCatalog(),
            status: memoryStatus.memoryStrategyStatus || null
        };
    });
    ipcMain.handle('ailis:memory-set-strategy', async (_event, payload = {}) =>
        ensureAILISGateway().setMemoryStrategy(payload.strategy || payload.id || '')
    );
    ipcMain.handle('ailis:memory-cognition-status', async () =>
        ensureAILISGateway().getMemoryCognitionStatus()
    );
    ipcMain.handle('ailis:memory-cognition-curate', async (_event, payload = {}) =>
        ensureAILISGateway().curateMemoryCognition(payload || {})
    );
    ...
    ipcMain.on('ailis:begin-drag-pet-window', (event) => {
        ...
        const cursor = screen.getCursorScreenPoint();
        const baseBounds = petWindow.getBounds();
        petDragState = {
            cursor,
            baseBounds,
            lastAppliedBounds: { ...baseBounds },
            lastDialogueUpdateAt: 0
        };
    ...
    ipcMain.on('ailis:chat-send-message', (_event, payload = {}) => {
        personaHostWindow?.webContents.send('ailis:chat-send-message', payload);
        showChatWindow();
    });

    ipcMain.on('ailis:chat-control', (_event, payload = {}) => {
        personaHostWindow?.webContents.send('ailis:chat-control', payload);
    });

    ipcMain.on('ailis:pet-chat-event', (event, payload = {}) => {
        const sourceWindow = BrowserWindow.fromWebContents(event.sender);
        if (!sourceWindow || sourceWindow !== personaHostWindow) {
            return;
        }
        if (chatWindow) {
    ...
    ipcMain.on('ailis:character-renderer-command', (event, payload = {}) => {
        const sourceWindow = BrowserWindow.fromWebContents(event.sender);
        if (!sourceWindow || sourceWindow !== personaHostWindow) {
            return;
        }
        ...
        if (useUnity) {
            sendCharacterRendererCommand(payload);
            return;
        }
        petWindow?.webContents.send('ailis:character-renderer-command', payload);
    });

    ipcMain.on('ailis:dialogue-surface-event', (event, payload = {}) => {
        const sourceWindow = BrowserWindow.fromWebContents(event.sender);
        if (!sourceWindow || sourceWindow !== personaHostWindow) {
            return;
        }
        publishDialogueSurfaceEvent(payload);
    });

    ipcMain.on('ailis:chat-state-sync-request', () => {
        personaHostWindow?.webContents.send('ailis:chat-state-sync-request', {});
    });
}
...
if (!app.requestSingleInstanceLock()) {
    ...
    app.on('second-instance', () => {
        if (petWindow) {
            ...
            petWindow.focus();
            persistDesktopState();
            void syncCharacterRenderer('second_instance');
        }
        showChatWindow();
    });
}
...
app.whenReady().then(() => {
    const hasPersistedDesktopState = fs.existsSync(
        path.join(app.getPath('userData'), 'desktop-state.json')
    );
    desktopState = loadDesktopState(app);
    desktopState.preferences.characterRendererBackend = normalizeAvailableCharacterRendererBackend(
        hasPersistedDesktopState
            ? desktopState.preferences.characterRendererBackend
            : PRODUCT_VARIANT.defaultCharacterRendererBackend
    );
    desktopState.preferences.unityCharacterPackageId = normalizeUnityCharacterPackageId(
        desktopState.preferences.unityCharacterPackageId || DEFAULT_UNITY_CHARACTER_PACKAGE_ID
    );
    ...
    const restoreCharacterAfterDisplayChange = () => {
        setTimeout(() => {
            restorePetWindowToVisibleDisplay({ persist: true });
            syncCharacterRendererWindowBounds();
            updateDialogueSurfacePosition();
    ...
    screen.on('display-removed', restoreCharacterAfterDisplayChange);
    screen.on('display-metrics-changed', restoreCharacterAfterDisplayChange);
    ...
    app.on('activate', () => {
        if (BrowserWindow.getAllWindows().length === 0) {
            ...
            petWindow.show();
            persistDesktopState();
            void syncCharacterRenderer('app_activated');
        }
    });
});
...
app.on('before-quit', () => {
    ...
    const characterRendererShutdown = characterRendererRuntime?.stop?.('app_quit');
    characterRendererShutdown?.catch?.(() => {});
    closeCosyVoice3TTS();
});
