...
// The control panel needs the renderer selection before its first paint; this
// compact preference snapshot avoids flashing the obsolete backend controls.
...
contextBridge.exposeInMainWorld('ailisDesktop', {
    ...
    characterLab: {
        ...
        selectCharacter: (payload) => ipcRenderer.invoke(
            'ailis:character-lab-select-character',
            payload || {}
        ),
    ...
    onPetRendererState: (listener) => {
        const wrapped = (_event, payload = {}) => listener(payload);
        ipcRenderer.on('ailis:pet-renderer-state', wrapped);
        return () => ipcRenderer.removeListener('ailis:pet-renderer-state', wrapped);
    },
