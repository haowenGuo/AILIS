...
const CHARACTER_RENDERER_BACKEND_OPTIONS = ['electron', 'unity'];
const DEFAULT_CHARACTER_RENDERER_BACKEND = 'electron';
const DEFAULT_UNITY_CHARACTER_PACKAGE_ID = '';
...
function normalizeUnityRendererSettings(value = {}) {
    ...
    const normalizeOption = (candidate, options, fallback) => {
        const normalized = String(candidate || '').trim().toLowerCase();
        return options.includes(normalized) ? normalized : fallback;
    };
    const normalizeHexColor = (candidate, fallback) => {
        const normalized = String(candidate || '').trim().toUpperCase();
        return /^#[0-9A-F]{6}$/.test(normalized) ? normalized : fallback;
    };
    const requestedMsaa = Math.round(Number(input.msaaSampleCount ?? input.antiAliasing));
    const msaaSampleCount = [1, 2, 4, 8].includes(requestedMsaa)
        ? requestedMsaa
        : defaults.msaaSampleCount;
    ...
    const pipelineAsset = normalizeOption(
        legacyPipelineAsset,
    ...
    return {
        ...
        targetFrameRate: legacyPerformancePreset
            ? 60
            : clampNumber(input.targetFrameRate, 24, 240, defaults.targetFrameRate, 0),
        renderScale: clampNumber(input.renderScale, 0.5, 2, defaults.renderScale, 2),
        msaaSampleCount,
        adaptiveSupersampling: normalizeBoolean(
            input.adaptiveSupersampling,
            defaults.adaptiveSupersampling
        ),
        shadowDistance: clampNumber(input.shadowDistance, 0, 100, defaults.shadowDistance, 1),
        shadowCascadeCount: clampNumber(input.shadowCascadeCount, 1, 4, defaults.shadowCascadeCount, 0),
        cameraAntialiasing: normalizeOption(
            input.cameraAntialiasing,
            ['none', 'fxaa', 'smaa'],
            defaults.cameraAntialiasing
        ),
        cameraAntialiasingQuality: normalizeOption(
            input.cameraAntialiasingQuality,
            ['low', 'medium', 'high'],
            defaults.cameraAntialiasingQuality
        ),
        renderPostProcessing: normalizeBoolean(
            input.renderPostProcessing,
            defaults.renderPostProcessing
        ),
        ...
        cameraFramingMode: normalizeOption(
            input.cameraFramingMode,
            ['full-body', 'manual'],
            defaults.cameraFramingMode
        ),
        ...
        mainLightShadows: normalizeBoolean(
            input.mainLightShadows,
        ...
        keyLightColor: normalizeHexColor(input.keyLightColor, defaults.keyLightColor),
        fillLightIntensity: clampNumber(input.fillLightIntensity, 0, 2, defaults.fillLightIntensity, 2),
        fillLightColor: normalizeHexColor(input.fillLightColor, defaults.fillLightColor),
        rimLightIntensity: clampNumber(input.rimLightIntensity, 0, 2, defaults.rimLightIntensity, 2),
        rimLightColor: normalizeHexColor(input.rimLightColor, defaults.rimLightColor)
    };
}
...
function normalizeCharacterRendererBackend(value) {
    const normalized = String(value || '').trim().toLowerCase();
    return CHARACTER_RENDERER_BACKEND_OPTIONS.includes(normalized)
        ? normalized
        : DEFAULT_CHARACTER_RENDERER_BACKEND;
}

function normalizeUnityCharacterPackageId(value) {
...
function getDefaultState() {
    ...
    return {
        ...
        preferences: {
            petSkipTaskbar: true,
            petScale,
            characterRendererBackend: DEFAULT_CHARACTER_RENDERER_BACKEND,
            unityCharacterPackageId: DEFAULT_UNITY_CHARACTER_PACKAGE_ID,
            speechMode: 'off',
...
function normalizeState(inputState) {
    ...
    normalizedState.preferences.characterRendererBackend = normalizeCharacterRendererBackend(
        normalizedState.preferences.characterRendererBackend
    );
    normalizedState.preferences.unityCharacterPackageId = normalizeUnityCharacterPackageId(
        normalizedState.preferences.unityCharacterPackageId
    );
    ...
    if ((nextState.version || 0) < 33) {
        normalizedState.preferences.unityRenderer = {
            ...(normalizedState.preferences.unityRenderer || {}),
            ambientIntensity: DEFAULT_UNITY_RENDERER_SETTINGS.ambientIntensity,
            mainLightShadowStrength: DEFAULT_UNITY_RENDERER_SETTINGS.mainLightShadowStrength,
            keyLightIntensity: DEFAULT_UNITY_RENDERER_SETTINGS.keyLightIntensity,
            keyLightYaw: DEFAULT_UNITY_RENDERER_SETTINGS.keyLightYaw,
            keyLightPitch: DEFAULT_UNITY_RENDERER_SETTINGS.keyLightPitch,
            keyLightColor: DEFAULT_UNITY_RENDERER_SETTINGS.keyLightColor,
            fillLightIntensity: DEFAULT_UNITY_RENDERER_SETTINGS.fillLightIntensity,
            fillLightColor: DEFAULT_UNITY_RENDERER_SETTINGS.fillLightColor,
            rimLightIntensity: DEFAULT_UNITY_RENDERER_SETTINGS.rimLightIntensity,
            rimLightColor: DEFAULT_UNITY_RENDERER_SETTINGS.rimLightColor
        };
    }
    normalizedState.preferences.unityRenderer = normalizeUnityRendererSettings(
        normalizedState.preferences.unityRenderer
    );
...
module.exports = {
    CHARACTER_RENDERER_BACKEND_OPTIONS,
