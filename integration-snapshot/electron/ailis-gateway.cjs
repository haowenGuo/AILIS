...
const { AILISWorldCompanionRuntime } = require('./ailis-world-companion.cjs');
...
class AILISGateway extends EventEmitter {
    ...
    getStatus(options = {}) {
        ...
        return {
            ...
            worldCompanion: this.worldCompanionRuntime?.getStatus?.() || {
                enabled: true,
    ...
    ensureWorldCompanionRuntime() {
        if (!this.worldCompanionRuntime) {
            this.worldCompanionRuntime = new AILISWorldCompanionRuntime({
                gateway: this,
                getAgentRunner: () => this.ensureAgentRunner(),
                getLlmSettings: () => this.getLlmSettings?.() || {}
            });
        }
        return this.worldCompanionRuntime;
    }
    ...
    async handleHttpRequest(req, res) {
        ...
        if (url.pathname === '/world/companion/run' && req.method === 'POST') {
            const body = await this.readJsonBody(req);
            this.sendJson(res, 200, await this.runWorldCompanion(body));
            return;
        }

        if (url.pathname === '/world/companion/interrupt' && req.method === 'POST') {
            const body = await this.readJsonBody(req);
            this.sendJson(res, 200, await this.interruptWorldCompanion(body));
            return;
        }

        if (url.pathname === '/world/tts' && req.method === 'POST') {
            const body = await this.readJsonBody(req);
            this.sendJson(res, 200, await this.synthesizeWorldSpeech(body));
            return;
        }

        if (url.pathname === '/world/asr' && req.method === 'POST') {
            const body = await this.readJsonBody(req);
            this.sendJson(res, 200, await this.transcribeWorldSpeech(body));
            return;
        }
    ...
    async runWorldCompanion(request = {}) {
        return await this.ensureWorldCompanionRuntime().run(
            request && typeof request === 'object' ? request : {}
        );
    }

    async interruptWorldCompanion(request = {}) {
        return await this.ensureWorldCompanionRuntime().interrupt(
            request && typeof request === 'object' ? request : {}
        );
    }

    async synthesizeWorldSpeech(request = {}) {
        const input = request && typeof request === 'object' ? request : {};
        const turnId = normalizeString(input.turnId);
        const text = normalizeString(input.text || input.speechText);
        if (!turnId || !text || text.length > 8000) {
            throw new GatewayHttpError(
                400,
                'invalid_world_tts_request',
                'turnId and text are required; text must be at most 8000 characters.'
            );
        }
        if (typeof this.voiceServices.tts !== 'function') {
            throw new GatewayHttpError(503, 'world_tts_unavailable', 'AILIS TTS service is unavailable.');
        }
        const result = await this.voiceServices.tts({
            ...input,
            text,
            speechText: text,
        ...
        return {
            ...
            ok: result?.ok !== false && Boolean(normalizeString(result?.audio_base64)),
            turnId
        };
    }

    async transcribeWorldSpeech(request = {}) {
        const input = request && typeof request === 'object' ? request : {};
        const captureId = normalizeString(input.captureId);
        const audioBase64 = normalizeString(input.audioBase64);
        if (!captureId || !audioBase64) {
            throw new GatewayHttpError(
                400,
                'invalid_world_asr_request',
                'captureId and audioBase64 are required.'
            );
        }
        const audioBytes = Buffer.from(audioBase64, 'base64');
        if (!audioBytes.length || audioBytes.length > 12 * 1024 * 1024) {
            throw new GatewayHttpError(
                413,
                'world_asr_audio_too_large',
                'Decoded audio must contain between 1 byte and 12 MiB.'
            );
        }
        if (typeof this.voiceServices.asr !== 'function') {
            throw new GatewayHttpError(503, 'world_asr_unavailable', 'AILIS ASR service is unavailable.');
        }
        const result = await this.voiceServices.asr(audioBytes, input);
        const normalized = result && typeof result === 'object'
            ? result
            : { text: normalizeString(result) };
        return {
            ...normalized,
            ok: normalized.ok !== false && Boolean(normalizeString(normalized.text || normalized.transcript)),
            captureId
        };
    }
