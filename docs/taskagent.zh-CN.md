...
历史 A7 基线把经过边界控制的工具输出保留在规范历史中，只在有效上下文预算进入硬压力区后启动语义压缩。它不会在最后一轮用独立的“四步摘要 Prompt”替换原历史。v1.4.1 的上下文与工具运行时变化及证据边界见[版本说明](releases/v1.4.1.md)；回退压缩现会同时保留 developer 角色和旧 user 角色的附件上下文封装。
