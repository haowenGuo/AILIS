...
const {
    createAILISDesktopPlatformAdapter
} = require('../electron/ailis-desktop-platform-adapter.cjs');
...
test('desktop platform adapter clamps window bounds inside the matching display', () => {
    ...
    const adapter = createAILISDesktopPlatformAdapter({
        screen: {
...
test('desktop platform adapter applies the configured app icon to windows', () => {
    MockBrowserWindow.created = [];
    const adapter = createAILISDesktopPlatformAdapter({
        BrowserWindow: MockBrowserWindow,
        icon: 'electron/assets/ailis-icon.png'
    });

    const window = adapter.createWindow({
        title: 'AILIS Test Window'
    });

    assert.equal(MockBrowserWindow.created[0], window);
    assert.equal(window.options.icon, 'electron/assets/ailis-icon.png');
});
