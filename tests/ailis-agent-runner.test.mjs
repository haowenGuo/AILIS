...
test('AILIS AgentRunner propagates RetrievalRequest through the async memory path', async () => {
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ailis-agent-retrieval-request-'));
    let capturedMemoryInput = null;
    let capturedCompilerInput = null;
    const runner = new AILISAgentRunner({
        gateway: {
            workspaceRoot: rootDir,
            auditDir: path.join(rootDir, 'audit'),
        ...
        memoryRuntime: {
            async getContextSourcesAsync(input) {
                capturedMemoryInput = input;
                return { relevantMemoriesText: '- retrieved evidence' };
            }
        },
        contextCompiler: {
            compile(input) {
                capturedCompilerInput = input;
                return 'compiled-memory';
            }
        }
    });
    ...
    const context = await runner.compileMemoryContextAsync({
        sessionId: 'eval-task',
        message: 'full generation prompt with output instructions',
        request: {
    ...
    assert.equal(context, 'compiled-memory');
    assert.equal(capturedMemoryInput.message, 'full generation prompt with output instructions');
    assert.equal(capturedMemoryInput.retrievalRequest.query, retrievalRequest.query);
    assert.equal(capturedMemoryInput.questionTime, retrievalRequest.referenceTime);
    assert.equal(capturedCompilerInput.retrievalRequest.query, retrievalRequest.query);
    assert.equal(capturedCompilerInput.memorySources.relevantMemoriesText, '- retrieved evidence');
});
