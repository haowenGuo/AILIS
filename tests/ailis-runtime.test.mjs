...
test('AILIS Gateway exposes runtime tools, update_plan, policy checks, and transcripts', async () => {
    ...
    try {
        ...
        assert.equal(plan.body.result.details.status, 'completed');
        assert.equal(plan.body.result.structuredContent.status, 'completed');
        assert.equal(plan.body.result.content[0].text, 'Plan updated.');
