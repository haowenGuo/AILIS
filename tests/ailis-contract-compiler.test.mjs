...
const {
    ...
    lintAilisContract
} = require('../electron/ailis-contract-compiler.cjs');
...
test('Tool Acquisition Gateway stores accepted and rejected compiled contracts', async () => {
    const workspaceRoot = await makeWorkspace('ailis-contract-intake-');
...
test('Tool Acquisition Gateway bulk exposes external Composio OpenAPI and live MCP specs', async () => {
    const workspaceRoot = await makeWorkspace('ailis-external-exposure-');
    const calls = [];
    const gateway = new AILISToolAcquisitionGateway({
        workspaceRoot,
        projectRoot: workspaceRoot,
...
test('Tool Acquisition Gateway executes trusted read-only OpenAPI exposure', async () => {
    const workspaceRoot = await makeWorkspace('ailis-openapi-executor-');
...
test('Tool Acquisition Gateway classifies OpenAPI HTTP rate limits with recovery hints', async () => {
    const workspaceRoot = await makeWorkspace('ailis-openapi-rate-limit-');
...
test('Tool Acquisition Gateway classifies OpenAPI forbidden responses with recovery hints', async () => {
    const workspaceRoot = await makeWorkspace('ailis-openapi-forbidden-');
    const server = await withHttpServer((_req, res) => {
        res.statusCode = 403;
...
test('Tool Acquisition Gateway exposes callable OpenAPI adapters as virtual direct tools', async () => {
    const workspaceRoot = await makeWorkspace('ailis-openapi-virtual-direct-');
    const server = await withHttpServer((req, res) => {
        const url = new URL(req.url, 'http://127.0.0.1');
        res.setHeader('content-type', 'application/json');
        res.end(JSON.stringify({
...
test('Tool Acquisition Gateway includes accepted contract intake in external search', async () => {
    const workspaceRoot = await makeWorkspace('ailis-contract-intake-search-');
    const gateway = new AILISToolAcquisitionGateway({
        workspaceRoot,
        projectRoot: workspaceRoot,
        stateDir: path.join(workspaceRoot, '.state', 'tool-acquisition')
...
test('Tool Acquisition Gateway includes built-in public OpenAPI tools in external search without prior exposure', async () => {
    const workspaceRoot = await makeWorkspace('ailis-builtin-openapi-search-');
    const gateway = new AILISToolAcquisitionGateway({
        workspaceRoot,
        projectRoot: workspaceRoot,
        stateDir: path.join(workspaceRoot, '.state', 'tool-acquisition')
...
test('Tool Acquisition Gateway executes approved OpenAPI adapter with env auth profile', async () => {
    const workspaceRoot = await makeWorkspace('ailis-openapi-adapter-auth-');
...
test('Tool Acquisition Gateway executes approved Composio adapter with scoped env auth', async () => {
    const workspaceRoot = await makeWorkspace('ailis-composio-adapter-auth-');
...
test('Capability Manager exposes contract compiler actions', async () => {
    const workspaceRoot = await makeWorkspace('ailis-contract-capability-');
