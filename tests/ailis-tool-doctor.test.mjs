...
test('Tool Doctor discovers MCP candidates from configs and local packages', async () => {
    const workspaceRoot = await makeWorkspace('ailis-tool-doctor-discovery-');
...
test('Tool Doctor records scorecards and gates repair proposals without applying patches', async () => {
    const workspaceRoot = await makeWorkspace('ailis-tool-doctor-scorecard-');
    const doctor = new AILISToolDoctor({
        workspaceRoot,
        projectRoot: workspaceRoot,
        auditDir: path.join(workspaceRoot, '.audit')
    ...
    const repair = await doctor.execute({
        ...
        validationCommands: ['pnpm ailis:mcp-soak']
    });
...
test('AILIS runtime exposes Tool Doctor as a runtime tool', async () => {
    const workspaceRoot = await makeWorkspace('ailis-tool-doctor-runtime-');
