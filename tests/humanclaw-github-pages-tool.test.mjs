...
const { executeGitHubPagesTool, parseGitHubRemote } = require('../electron/humanclaw-github-pages-tool.cjs');
...
test('GitHub Pages tool reports dist publish blockers without treating diagnostics as a tool crash', async (t) => {
    ...
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'humanclaw-github-pages-'));
