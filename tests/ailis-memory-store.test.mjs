...
test('AILIS lexical memory v2 searches active history beyond the 500-event state window', async () => {
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ailis-memory-lexical-full-history-'));
    try {
        const memoryRoot = path.join(rootDir, 'memory');
        const memory = new AILISMemoryRuntime({
            rootDir: memoryRoot,
        ...
        const events = Array.from({ length: 525 }, (_, index) => ({
            ...
            ts: new Date(Date.UTC(2025, 0, 1, 0, index)).toISOString(),
            sessionId: `session-${index}`,
            source: 'test',
            type: 'turn',
            userText: index === 0
                ? 'The archival launch codename was heliotrope sentinel.'
                : `Ordinary unrelated conversation ${index}.`,
            assistantText: index === 0
                ? 'Heliotrope sentinel is the durable historical evidence.'
                : 'Acknowledged.',
            summary: '',
            tags: [],
            importance: 1
        ...
        await fs.appendFile(
            path.join(memoryRoot, 'events.jsonl'),
            `${events.map((event) => JSON.stringify(event)).join('\n')}\n`,
            'utf8'
        );
        memory.state.stats.turnCount = events.length;

        const result = memory.searchMemory('What was the archival launch codename?', { limit: 4 });
        assert.equal(result.ok, true);
        assert.equal(result.strategy, 'bm25_phrase_v2');
        assert.equal(result.events[0]?.id, 'full-history-0');
        assert.equal(result.diagnostics.indexedEventCount, 525);
        assert.equal(result.diagnostics.sessionRepeatPenalty, 0.2);
        assert.ok(Number.isFinite(result.diagnostics.elapsedMs));
...
test('AILIS lexical memory v2 retrieves Chinese, Japanese, and Korean evidence without an LLM', async () => {
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ailis-memory-lexical-multilingual-'));
    try {
        const memory = new AILISMemoryRuntime({
            rootDir: path.join(rootDir, 'memory'),
            workspaceRoot: rootDir,
        ...
        const examples = [
            ['zh', '我最喜欢的咖啡是埃塞俄比亚耶加雪菲。', '记住了你的咖啡偏好。'],
            ['ja', '日本語プロジェクトの名前は星明かりです。', 'プロジェクト名を記録しました。'],
            ['ko', '한국어 프로젝트 이름은 별빛입니다.', '프로젝트 이름을 기록했습니다.']
        ];
        for (const [sessionId, userMessage, assistantMessage] of examples) {
            memory.recordTurn({ sessionId, userMessage, assistantMessage, source: 'test' });
        }

        const cases = [
            ['我喜欢什么咖啡？', 'zh'],
            ['日本語プロジェクトの名前は何ですか？', 'ja'],
            ['한국어 프로젝트 이름이 무엇인가요?', 'ko']
        ];
        for (const [query, expectedSession] of cases) {
            const result = memory.searchMemory(query, { limit: 2 });
            assert.equal(result.strategy, 'bm25_phrase_v2');
            assert.equal(result.events[0]?.sessionId, expectedSession);
        }
    } finally {
        await fs.rm(rootDir, { recursive: true, force: true });
    }
...
test('AILIS lexical memory v2 uses soft session diversity instead of dropping a third strong fact', async () => {
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ailis-memory-lexical-session-evidence-'));
    try {
        const memory = new AILISMemoryRuntime({
            rootDir: path.join(rootDir, 'memory'),
            workspaceRoot: rootDir,
        ...
        for (let index = 1; index <= 4; index += 1) {
            memory.recordTurn({
                sessionId: 'multi-fact-session',
                userMessage: `Orchid archive evidence part ${index} contains a required clue.`,
                assistantMessage: `Recorded orchid archive clue ${index}.`,
                source: 'test'
            });
        }

        const result = memory.searchMemory('List every orchid archive evidence clue.', { limit: 4 });
        assert.equal(result.events.length, 4);
        assert.ok(result.events.every((event) => event.sessionId === 'multi-fact-session'));
...
test('AILIS memory retrieval prefers an explicit RetrievalRequest over prompt and history text', async () => {
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ailis-memory-retrieval-request-'));
    const memory = new AILISMemoryRuntime({
        rootDir: path.join(rootDir, 'memory'),
        workspaceRoot: rootDir
    });
    ...
    memory.searchMemory = (query, options) => {
        ...
        return { strategy: 'bm25_phrase_v1', events: [] };
    };

    const sources = memory.getContextSources({
        sessionId: 'retrieval-request-test',
        message: 'Please answer with a long formatting contract.',
        messageHistory: [{ role: 'user', content: 'irrelevant old conversation' }],
        retrievalRequest: {
    ...
    assert.deepEqual(captured, {
        ...
        options: {
            limit: 8,
            questionTime: '2023-04-10T23:07:00.000Z'
        }
    });
    assert.equal(sources.retrievalRequestSchema, 'ailis.retrieval_request.v1');
    assert.equal(sources.retrievalRequestSource, 'benchmark_public_question');
    assert.equal(sources.retrievalQueryChars, 'Which tea do I always choose?'.length);
});
