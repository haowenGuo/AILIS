...
describe('Codex model bridge process lifecycle', () => {
    ...
    it('retries only transient model-only transport failures and caps attempts', () => {
        ...
        assert.equal(shouldRetryCodexBridgeFailure({
            ...
            error: 'failed to start turn: internal error; agent loop died unexpectedly',
            details: {
        ...
        assert.equal(shouldRetryCodexBridgeFailure({
            code: 'codex_app_server_protocol_error',
            error: 'failed to start turn: invalid output schema',
            details: {
                rpcMethod: 'turn/start',
                rpcCode: -32603
            }
    ...
    it('restarts the ephemeral app-server once when turn/start reports a dead agent loop', async () => {
        const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'ailis-codex-dead-loop-'));
        const codexHome = path.join(fixtureRoot, 'codex-home');
        const counterPath = path.join(fixtureRoot, 'attempts.txt');
        const entrypoint = path.join(fixtureRoot, 'fake-codex.cjs');
        const previousCounterPath = process.env.AILIS_CODEX_BRIDGE_TEST_COUNTER;
        await fs.mkdir(codexHome, { recursive: true });
        await fs.writeFile(path.join(codexHome, 'auth.json'), '{}', 'utf8');
        await fs.writeFile(entrypoint, `
const fs = require('node:fs');
const readline = require('node:readline');
const counterPath = process.env.AILIS_CODEX_BRIDGE_TEST_COUNTER;
...
lines.on('line', (line) => {
    ...
    if (message.method === 'thread/start') {
        send({ id: message.id, result: { thread: { id: 'thread-' + attempt }, instructionSources: [] } });
        return;
    }
    ...
    const turn = { id: 'turn-' + attempt, status: 'completed' };
    send({ id: message.id, result: { turn } });
    send({ method: 'turn/started', params: { threadId: 'thread-' + attempt, turn } });
    send({
        method: 'item/completed',
        params: {
            threadId: 'thread-' + attempt,
            turnId: turn.id,
            item: { id: 'message-1', type: 'agentMessage', text: '{"content":"recovered","finish_reason":"stop","tool_calls":[]}' }
        }
    });
    send({
        method: 'thread/tokenUsage/updated',
        params: { threadId: 'thread-' + attempt, tokenUsage: { last: { inputTokens: 3, outputTokens: 2 } } }
    });
    send({ method: 'turn/completed', params: { threadId: 'thread-' + attempt, turn } });
});
`, 'utf8');

        process.env.AILIS_CODEX_BRIDGE_TEST_COUNTER = counterPath;
        try {
            const result = await callCodexModelBridge({
                codexEntrypoint: entrypoint,
                codexHome,
                codexBridgeMaxAttempts: 2,
                timeoutMs: 10000
            }, { tools: [] }, [{ role: 'user', content: 'recover' }]);

            assert.equal(result.ok, true);
            assert.equal(result.content, 'recovered');
            assert.equal(result.providerMessage.bridgeAttempts, 2);
            assert.equal(result.providerMessage.retryCount, 1);
            assert.deepEqual(
                result.providerMessage.retriedFailureCodes,
                ['codex_app_server_protocol_error']
            );
            assert.equal(await fs.readFile(counterPath, 'utf8'), '2');
        } finally {
            if (previousCounterPath === undefined) {
                delete process.env.AILIS_CODEX_BRIDGE_TEST_COUNTER;
            } else {
                process.env.AILIS_CODEX_BRIDGE_TEST_COUNTER = previousCounterPath;
            }
