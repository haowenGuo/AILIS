...
test('AILIS humanlike eval relationship stages match product affinity design', () => {
...
test('AILIS humanlike eval seed dataset is structurally valid', async () => {
    const scenarios = await readJsonl(path.resolve('evals/ailis-humanlike/scenarios.jsonl'));
...
test('AILIS long-term companionship scenario set covers durable relationship risks', async () => {
    const scenarios = await readJsonl(path.resolve('evals/ailis-humanlike/long-term-companionship.scenarios.jsonl'));
    const validation = validateScenarioDataset(scenarios);
    assert.equal(validation.ok, true, JSON.stringify(validation.issues, null, 2));
...
test('AILIS 30-day longitudinal companionship benchmark has deep daily histories', async () => {
    const rawDataset = JSON.parse(await fs.readFile(path.resolve('evals/ailis-humanlike/longitudinal-companionship-30d.dataset.json'), 'utf8'));
    const scenarios = await readJsonl(path.resolve('evals/ailis-humanlike/longitudinal-companionship-30d.scenarios.jsonl'));
    const validation = validateScenarioDataset(scenarios);
    assert.equal(validation.ok, true, JSON.stringify(validation.issues, null, 2));
...
test('AILIS humanlike eval preserves longitudinal benchmark context for judge packets', () => {
...
test('AILIS humanlike eval 1000-scenario plan is balanced and explicit', async () => {
    const plan = JSON.parse(await fs.readFile(path.resolve('evals/ailis-humanlike/dataset-plan.json'), 'utf8'));
    const scenarios = await readJsonl(path.resolve('evals/ailis-humanlike/scenarios.jsonl'));
...
test('AILIS humanlike judge prompt contains core metrics and scenario context', () => {
    const scenario = normalizeScenario({
        id: 'close-test',
...
test('AILIS humanlike eval parses AILIS control tags like the frontend payload', () => {
...
test('AILIS humanlike judge parser accepts flat metric fields and legacy hard fail flags', () => {
...
test('AILIS humanlike judge packet is portable for strong manual judge flow', () => {
    const packet = buildHumanlikeJudgePacket({
        scenario: {
            id: 'packet-test',
...
test('AILIS humanlike judge parser scores weighted results and hard failures', () => {
    const response = JSON.stringify({
        scenario_id: 'judge-test',
        overall_comment: '自然但暴露了内部好感度。',
        metrics: {
            persona_consistency: { score: 4, reason: '像 AILIS' },
...
test('AILIS humanlike imported judgment normalizes JSONL rows', () => {
    const imported = normalizeImportedJudgment({
        ...
        judge_response: JSON.stringify({
            ...
            overall_comment: '整体自然。',
            metrics: {
                persona_consistency: { score: 4, reason: '稳定像 AILIS' },
                naturalness: { score: 4, reason: '自然' },
