...
const { AILISSelfDebugger } = require('../electron/ailis-self-debugger.cjs');
...
test('Self Debugger opens a case, collects evidence, validates a repair, and applies only after approval', async () => {
    const workspaceRoot = await makeWorkspace('ailis-self-debugger-');
...
test('AILIS runtime exposes Self Debugger as a high-risk runtime tool', async () => {
    const workspaceRoot = await makeWorkspace('ailis-self-debugger-runtime-');
    const runtime = new AILISRuntime({
        workspaceRoot,
        projectRoot: workspaceRoot,
...
test('Self Debugger rejects paths outside the project root during source evidence collection', async () => {
    const workspaceRoot = await makeWorkspace('ailis-self-debugger-path-');
    const debuggerTool = new AILISSelfDebugger({
        workspaceRoot,
        projectRoot: workspaceRoot,
        auditDir: path.join(workspaceRoot, '.audit')
    });
