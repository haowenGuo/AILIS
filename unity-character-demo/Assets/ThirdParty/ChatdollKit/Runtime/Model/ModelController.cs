...
namespace ChatdollKit.Model
{
    public class ModelController : MonoBehaviour
    {
        ...
        public bool FollowAvatarPosition = true;
        public bool ResetAdditionalAnimatorLayers = true;
        ...
        private Dictionary<string, int> lastIdleIndexes = new Dictionary<string, int>();
        ...
        public event Action<Animation, string> AnimationStarted;
        ...
        public Func<string, Dictionary<string, object>, CancellationToken, Task<AudioClip>> SpeechSynthesizerFunc
        {
        ...
        private void LateUpdate()
        {
            ...
            if (FollowAvatarPosition && AvatarModel != null)
            {
...
#region Idling
        ...
        // Change idling mode
        public async Task ChangeIdlingModeAsync(string mode = "normal", Func<Task> onBeforeChange = null)
        {
            if (!idleAnimations.ContainsKey(mode) ||
                !idleWeightedIndexes.ContainsKey(mode) ||
                idleWeightedIndexes[mode].Count == 0)
            {
                if (mode != "normal" &&
                    idleAnimations.ContainsKey("normal") &&
                    idleWeightedIndexes.ContainsKey("normal") &&
                    idleWeightedIndexes["normal"].Count > 0)
                {
                    Debug.LogWarning($"Idle mode is not registered: {mode}. Falling back to normal.");
                    mode = "normal";
                }
                else
                {
                    Debug.LogWarning($"Idle mode has no animations: {mode}.");
                    return;
                }
            }
            ...
            if (request.StartIdlingOnEnd && !token.IsCancellationRequested)
            {
                ...
                if (GetAnimation != GetIdleAnimation)
                {
                    ...
                    if (currentAnimation != null)
                    {
                        ...
                        if (remainingTime > 0)
                        {
                            await Task.Delay((int)(remainingTime * 1000), token);
                        }
                    }
...
#region Animation
        ...
        {
            if (!ResetAdditionalAnimatorLayers)
            {
                return;
            }
        ...
        private void UpdateAnimation()
        {
            ...
            var currentAnimationExpired =
                currentAnimation != null &&
                animationStartAt > 0 &&
                Time.realtimeSinceStartup - animationStartAt > currentAnimation.Duration;
            if (currentAnimation == null ||
                animationToRun.Id != currentAnimation.Id ||
                animationStartAt == 0 ||
                currentAnimationExpired)
            {
                // Start new animation
                ResetLayers();
                if (!string.IsNullOrWhiteSpace(animationToRun.ParameterKey) &&
                    animator.parameters.Any(parameter =>
                        parameter.name == animationToRun.ParameterKey &&
                ...
                if (!string.IsNullOrEmpty(animationToRun.LayeredAnimationName))
                {
                    ...
                    var stateHash = Animator.StringToHash(animationToRun.LayeredAnimationName);
                    ...
                    {
                        Debug.LogWarning(
                            $"Animation state not found: {animationToRun.LayeredAnimationLayerName}/" +
                            animationToRun.LayeredAnimationName);
                    }
                }
                currentAnimation = animationToRun;
                animationStartAt = Time.realtimeSinceStartup;
                AnimationStarted?.Invoke(currentAnimation, IdlingMode);
            }
        }
        ...
        private Animation GetIdleAnimation()
        {
            if (currentAnimation == null || animationStartAt == 0 || Time.realtimeSinceStartup - animationStartAt > currentAnimation.Duration)
            {
                ...
                if (!idleAnimations.TryGetValue(IdlingMode, out var animations) ||
                    !idleWeightedIndexes.TryGetValue(IdlingMode, out var weightedIndexes) ||
                    weightedIndexes.Count == 0)
                {
                    return default;
                }

                // The configured weights already express how often each idle
                // should appear. Excluding the previous state forced mechanical
                // A/B alternation and defeated those authored probabilities.
                var weightedIndex = UnityEngine.Random.Range(0, weightedIndexes.Count);
                var animationIndex = weightedIndexes[weightedIndex];
                lastIdleIndexes[IdlingMode] = animationIndex;
...
#region Avatar
        public void ActivateAvatar(GameObject avatarObject = null, bool configureViseme = false)
        {
            ...
            animator = AvatarModel.GetComponentInChildren<Animator>();
