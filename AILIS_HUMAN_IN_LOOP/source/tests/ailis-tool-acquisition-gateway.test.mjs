...
test('Tool Acquisition Gateway searches core bundles and official MCP Registry candidates', async () => {
    const workspaceRoot = await makeWorkspace('ailis-tool-acquisition-');
    ...
    assert.equal(secure.install.authEnvVar, 'AILIS_MCP_IO_EXAMPLE_SECURE_MAIL_TOKEN');
    assert.equal(secure.install.mcpConfig.bearerTokenEnvVar, 'AILIS_MCP_IO_EXAMPLE_SECURE_MAIL_TOKEN');
...
test('Tool Acquisition Gateway records task-tool outcomes and recommends verified tools', async () => {
    const workspaceRoot = await makeWorkspace('ailis-tool-learning-');
    const gateway = new AILISToolAcquisitionGateway({
        workspaceRoot,
        projectRoot: workspaceRoot,
        stateDir: path.join(workspaceRoot, '.state', 'tool-acquisition'),
...
test('Capability Manager plans MCP Registry candidates through the Tool Acquisition Gateway', async () => {
    const workspaceRoot = await makeWorkspace('ailis-capability-acquisition-');
