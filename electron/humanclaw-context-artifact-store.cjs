...
function buildEndpointCriteria(args = {}, prefix = 'start', defaults = []) {
    ...
    const address = normalizeString(args[`${prefix}Address`] || args[`${prefix}_address`] || (parseCell(direct) ? direct : ''));
...
function computeSpreadsheetPath(record = {}, payload = {}, args = {}) {
    ...
    const blockedFills = normalizeList(args.blockedFills || args.blocked_fills || args.blockedColors || args.blocked_colors);
    ...
    const passableFills = normalizeList(args.passableFills || args.passable_fills || args.passableColors || args.passable_colors);
...
class HumanClawContextArtifactStore {
    constructor(options = {}) {
