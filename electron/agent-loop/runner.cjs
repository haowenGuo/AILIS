...
function buildAgentDecisionLowLatencyPayload(payload = {}, { settings = {}, requestContext = {} } = {}) {
    ...
    const taskRoutePending = requestContext.taskAgentRoutePending === true;
    const thinking = resolveExplicitAgentDecisionThinking(settings, requestContext) || (
        taskRoutePending && isDeepSeekAgentDecisionProvider(settings)
            ? { type: 'disabled' }
            : null
    );
    const defaultMaxTokens = taskRoutePending
        ? TASK_ROUTE_MAX_OUTPUT_TOKENS
        : localConstrainedProvider
            ? 320
            : 0;
    const maxTokens = Number(
        (taskRoutePending
            ? requestContext.taskRouteDecisionMaxTokens || settings.taskRouteDecisionMaxTokens
            : requestContext.agentDecisionMaxTokens || settings.agentDecisionMaxTokens) ||
            defaultMaxTokens
    );
    const nextPayload = {
        ...payload,
        ...
        parallel_tool_calls: taskRoutePending
            ? false
            : resolveParallelToolCalls(settings, requestContext),
        latencyProfile: taskRoutePending ? 'task_route_fast' : 'agent_decision_fast'
    };
    if (reasoningEffort) {
    ...
    if (Number.isFinite(maxTokens) && maxTokens > 0) {
        const minTokens = taskRoutePending ? 64 : localConstrainedProvider ? 64 : 256;
        const maxTokenLimit = taskRoutePending ? 256 : localConstrainedProvider ? 1024 : 8192;
...
function buildAgentTaskState({
    ...
    const execution = buildExecutionStateLane(stepResults);
...
function resolveDirectToolTransportTimeoutMs({
    toolId = '',
    args = {},
    agentWaitTimeoutMs = 0,
    taskHandoffTimeoutMs = 0
} = {}) {
    const requestedVerificationTimeoutMs = Number(args?.timeoutMs || args?.timeout_ms || 0);
    const verificationTransportTimeoutMs = toolId === TASK_VERIFY_TOOL_ID
        ? Math.max(
            DEFAULT_TASK_VERIFY_EXECUTION_TIMEOUT_MS,
            Number.isFinite(requestedVerificationTimeoutMs) && requestedVerificationTimeoutMs > 0
                ? requestedVerificationTimeoutMs
                : 0
        ) + TOOL_TRANSPORT_TIMEOUT_BUFFER_MS
        : 0;
    return Math.max(
        Number(agentWaitTimeoutMs) || 0,
        Number(taskHandoffTimeoutMs) || 0,
        verificationTransportTimeoutMs
    );
}
...
function resolveAgentDirectToolChoice({
    ...
    const routeAvailable = (Array.isArray(directToolSpecs) ? directToolSpecs : [])
        .some((spec) => canonicalDirectToolId(spec?.name || spec?.function?.name) === TASK_ROUTE_TOOL_NAME);
    const routeAlreadyDecided = (Array.isArray(stepResults) ? stepResults : [])
        .some((stepResult) => canonicalDirectToolId(stepResult?.tool) === TASK_ROUTE_TOOL_NAME);
    if (
        ...
        return { name: TASK_ROUTE_TOOL_NAME, required: true };
    }
...
function isDeepSeekAgentDecisionProvider(settings = {}) {
    return [settings.provider, settings.baseUrl, settings.model]
        .map((value) => normalizeText(value).toLowerCase())
        .some((value) => value.includes('deepseek'));
}
...
function validateAgentToolLoopGuard(step = {}, stepResults = [], requestContext = {}) {
    ...
    return validateDesktopExecutionLoop(step, stepResults, requestContext);
}
...
function buildTaskRunHandoffDisplayText(handoff = {}) {
    ...
    if (delivery.required === true && delivery.verified !== true) {
        const verification = delivery.latestVerification || null;
        lines.push(verification
            ? `验收状态：${verification.scope || '验证命令'}未通过（exitCode=${verification.exitCode ?? 'unknown'}${verification.outputId ? `，证据：${verification.outputId}` : ''}）。`
            : '验收状态：当前补丁尚未取得通过的测试、构建或检查记录。');
    }
...
function buildTaskRunHandoffPackage({
    ...
    const delivery = assessTaskDelivery({
        ...
        terminalStatus: handoffStatus,
        isMutatingTool: (toolId) => getToolContract(toolId)?.mutates === true
    });
    const recommendation = delivery.required && !delivery.verified
        ? delivery.latestVerification && !delivery.latestVerification.passed
            ? '根据验收失败证据修复实现，然后重新运行 task_verify。'
            : '运行与当前改动匹配的 task_verify，取得可复现的验收结果。'
        : failedSteps.length
    ...
    const packageObject = {
        ...
        ok: handoffStatus.startsWith('completed') || ['success', 'succeeded', 'verified_delivery'].includes(handoffStatus),
        runId,
        sessionId,
        ...
        unresolvedFields: [...new Set(
            [
                ...
                ...(Array.isArray(delivery.unresolved) ? delivery.unresolved : [])
            ]
...
const TASK_ROUTE_TOOL_NAME = 'task_route';
...
function buildLlmAgentDirectToolPrompt({
    ...
    const personaRuntimeInstructions = [
        ...
        !taskAgentRoutingOwned
            ...
            : '',
        requireTaskExecution && !taskAgentRoutingOwned
...
class AILISAgentRunner {
    ...
    async executeAgentToolStep({ runId, step, toolContext, request, iteration }) {
        ...
        const taskHandoffTimeoutMs = directToolId === PERSONA_HANDOFF_TOOL_ID
            ...
            : 0;
        const transportTimeoutMs = resolveDirectToolTransportTimeoutMs({
            toolId: directToolId,
            args: step.args,
            agentWaitTimeoutMs,
            taskHandoffTimeoutMs
        });
    ...
    async runLlmAgentLoop({
        ...
        const activeDeliveryPolicy = normalizeDeliveryPolicy(
            request.deliveryProtocol || requestContext.deliveryProtocol
        );
        const maxSteps = 0;
        ...
        // 真正控制“继续下一轮还是结束”的最小生产循环已经抽到 core-loop.cjs。
        // runIteration 只负责完成一整轮业务：Context → Decision → Action → Observation。
        const runIteration = async (iteration) => {
            // ── Round 1/5：处理中断、用户追加输入和 Agent 邮箱消息 ──────────────
            // 如果用户在任务执行期间又发来“继续”“速度”等内容，它们会在这里
            // 加入当前 Turn，而不是启动一个彼此不知道的新任务。
            ...
            // ── Round 2/5：构造本轮模型上下文和模型可见工具表 ─────────────────
            // Persona 与 TaskAgent 共用循环，但会根据 agentRuntimeRole 得到不同工具：
            // Persona 偏对话和任务移交；TaskAgent 才拥有实际执行任务的工具。
            ...
            const commonPromptArgs = {
                ...
                deliveryPolicy: request.deliveryProtocol || requestContext.deliveryProtocol || {},
                verificationEnvironment:
                    request.verificationEnvironment || requestContext.verificationEnvironment || null,
                taskAgentRoutingOwned: requestContext.taskAgentRoutingOwned === true,
                taskAgentRoutePending: requestContext.taskAgentRoutePending === true,
                personaRenderOnly: requestContext.personaRenderOnly === true,
            ...
            const taskRoutePending = requestContext.taskAgentRoutePending === true;
            const parallelToolCalls = taskRoutePending
                ? false
            ...
            // ── Round 4/5：处理不需要工具的终止决策 ───────────────────────────
            // final 会形成用户可见答案；blocked 会保留现场并说明无法继续的原因。
            if (decision.action === 'final') {
                ...
                if ((isTaskAgentRole(agentRuntimeRole) || isUnifiedAgentRole(agentRuntimeRole)) && activeDeliveryPolicy.requireVerification && !delivery.canDeliver) {
                    const gateSignature = JSON.stringify({
                        ...
                        latestVerificationId: delivery.latestVerification?.id || '',
                        latestVerificationStatus: delivery.latestVerification?.status || '',
                        stepCount: stepResults.length
                    });
                    repeatedDeliveryGateFinals = gateSignature === deliveryGateFinalSignature
                        ? repeatedDeliveryGateFinals + 1
                        : 1;
                    deliveryGateFinalSignature = gateSignature;
                    modelInputContextManager?.recordItems?.([
                        responseMessage('developer', buildDeliveryGateObservation(delivery))
                    ]);
                    events.push({
                        type: 'agent_delivery_gate',
                        status: delivery.status,
                        iteration,
                        delivery
                    });
                    await appendRuntimeItem({
                        type: 'agent.delivery_gate',
                        status: delivery.status,
                        payload: {
                            iteration,
                            delivery
                    ...
                    if (repeatedDeliveryGateFinals >= 2) {
                        const taskRunHandoff = buildTaskRunHandoffPackage({
                            status: 'blocked',
                            reason: 'delivery_verification_unresolved',
                            runId,
                            sessionId,
                            message: currentTurnRequest,
                            startedAt,
                            ...
                            latestDecision: decision,
                            finalAnswer: '',
                            partialAnswer: decision.summary || decision.finalAnswer || '',
                            unresolvedFields: delivery.unresolved,
                            contextManagerCheckpoint: contextManagerCheckpoint('delivery_gate_blocked', iteration),
                            deliveryPolicy: activeDeliveryPolicy
                        });
                        const displayText = taskRunHandoff.userVisibleSummary ||
                            '当前实现尚未通过验收，已保留修改、失败证据和后续建议。';
                        ...
                        return await finishRuntimeRun(attachPersonaSurface({
                            ...
                            status: 'blocked',
                            ok: false,
                            source: 'agent_delivery_gate',
                            expression: 'relaxed'
                        })), {
                            source: 'agent_delivery_gate',
                            nextAction: taskRunHandoff.recommendation || ''
                        });
                    }
                    return CONTINUE_AGENT_LOOP;
                }
                ...
                const baseTaskRunHandoff = buildTaskRunHandoffPackage({
                    status: delivery.verified ? 'verified_delivery' : 'completed',
                    reason: 'model_final',
                    runId,
                    sessionId,
                    message: currentTurnRequest,
            ...
            // ── Round 5/5：验证并执行工具，把结果记录为 Observation ──────────
            // 有多个互不冲突的工具调用时进入批量路径，否则继续走下面的单工具路径。
            ...
            // 单工具路径。step 包含模型选择的工具名、call id 和 arguments。
            ...
            // 真正执行工具。这里返回的是“观察结果”，而不是 Runner 自己编造的答案。
            // 下一轮模型会读取 stepResult，再自行判断任务是否已经完成。
            ...
            if (canonicalDirectToolId(step.tool) === TASK_ROUTE_TOOL_NAME) {
                ...
                const routeMode = normalizeText(routeDetails.mode || step.args?.mode).toLowerCase();
                if (stepResult.response?.ok === true && ['chat', 'execute'].includes(routeMode)) {
                    requestContext.taskAgentRoutePending = false;
                    events.push({
                        type: 'task_route',
                        status: 'completed',
                        iteration,
                    ...
                    this.gateway.emitGatewayEvent?.('task_agent.route.decided', {
                        runId,
                        sessionId,
                        status: 'completed',
                        mode: routeMode,
            ...
            if (
                ...
                const handoffOk = stepResult.response?.ok === true &&
                    (packetStatus.startsWith('completed') || ['success', 'succeeded', 'verified_delivery'].includes(packetStatus));
                return await finishRuntimeRun(attachPersonaSurface({
                    ok: handoffOk,
                    runId,
                    sessionId,
            ...
            // 工具结果已经由 recordToolOutputToContextManager 写入 Observation。
            // 返回专用信号后，core-loop.cjs 会把 iteration 加 1，再让模型读取结果。
            return CONTINUE_AGENT_LOOP;
        };

        // 从这里进入独立的生产 Core Loop。只要 runIteration 返回继续信号，
        // core-loop.cjs 就会增加轮次并再次调用它；其他结果会直接结束整个运行。
        return await runCoreAgentLoop({
            startIteration,
            runIteration
        });
