...
} = require('./ailis-memory-lexical-index.cjs');
const { normalizeRetrievalRequest } = require('./ailis-retrieval-request.cjs');
...
const MEMORY_SEARCH_SESSION_REPEAT_PENALTY = 0.2;
const MEMORY_SEARCH_EXTENDED_SCRIPT_PATTERN = /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}]/u;
...
function selectMemoryEntriesWithSoftSessionDiversity(
    scored = [],
    limit = 8,
    sessionRepeatPenalty = MEMORY_SEARCH_SESSION_REPEAT_PENALTY
) {
    const pool = [...scored];
    const selected = [];
...
class AILISMemoryRuntime {
    constructor(options = {}) {
        ...
        this.lexicalIndex = options.lexicalIndex || new AILISLexicalMemoryIndex({
    ...
    getStatus() {
        ...
        return {
            ...
            lexicalIndexStatus: this.lexicalIndex?.getStatus?.() || null,
    ...
    getContextSources({
        ...
        const resolvedRetrievalRequest = normalizeRetrievalRequest(retrievalRequest, {
            query: buildMemoryRetrievalQuery(message, messageHistory),
            referenceTime: questionTime,
            source: retrievalRequest ? 'explicit' : 'message_and_recent_history'
        });
        const retrievalQuery = resolvedRetrievalRequest.query;
        const searchResult = this.searchMemory(retrievalQuery, {
            limit: DEFAULT_RELEVANT_EVENT_LIMIT,
            questionTime: resolvedRetrievalRequest.referenceTime
        });
        return this.buildContextSources({
            ...
            contextMode,
            retrievalQuery,
            retrievalRequest: resolvedRetrievalRequest,
            searchResult
        });
    }
    ...
    buildContextSources({
        ...
        return {
            ...
            retrievalRequestSchema: normalizeText(retrievalRequest?.schema),
            retrievalRequestSource: normalizeText(retrievalRequest?.source),
    ...
    searchMemory(query, { limit = 10, strategy = this.memoryStrategy, questionTime = '' } = {}) {
        ...
        let useInMemoryLexicalV2 = false;
        if (resolvedStrategy === 'bm25_phrase_v2') {
            const stateEvents = Array.isArray(this.state?.events) ? this.state.events : [];
            const recordedTurnCount = Math.max(
                stateEvents.length,
                Number(this.state?.stats?.turnCount) || 0
            );
            const completeHistoryInMemory = recordedTurnCount <= stateEvents.length;
            const requiresExtendedScript = MEMORY_SEARCH_EXTENDED_SCRIPT_PATTERN.test(
                normalizeText(query)
            );
            if (completeHistoryInMemory && !requiresExtendedScript) {
                useInMemoryLexicalV2 = true;
            } else {
                const indexedResult = this.lexicalIndex?.search?.(query, { limit });
                if (indexedResult?.ok) {
                ...
                return {
                    ...
                    diagnostics: {
                        ...
                        requestedStrategy: 'bm25_phrase_v2',
                        fallbackStrategy: 'bm25_phrase_v1',
                        fallbackReason: indexedResult?.error || indexedResult?.status || 'lexical_index_unavailable'
                    }
                };
            }
        }
        if (resolvedStrategy !== 'bm25_phrase_v1' && !useInMemoryLexicalV2) {
        ...
        let selected;
        if (useInMemoryLexicalV2) {
            selected = selectMemoryEntriesWithSoftSessionDiversity(
                scored,
                boundedLimit
            );
        } else {
            ...
            if (selected.length < boundedLimit) {
                ...
                for (const entry of deferred) {
                    if (selectedIds.has(entry.event?.id)) continue;
                    selected.push(entry);
                    if (selected.length >= boundedLimit) break;
                }
            }
        ...
        return {
            ...
            events,
            strategy: useInMemoryLexicalV2 ? 'bm25_phrase_v2' : 'bm25_phrase_v1',
            diagnostics: useInMemoryLexicalV2
                ? {
                    implementationVersion: 'adaptive_lexical_v2',
                    indexBackend: 'in_memory_bm25_mmr_v2',
                    completeHistoryInMemory: true,
    ...
    async searchMemoryAsync(query, {
        ...
        const resolvedStrategy = resolveMemoryStrategy(strategy, this.memoryStrategy);
        if (resolvedStrategy === 'bm25_phrase_v1' || resolvedStrategy === 'bm25_phrase_v2') {
