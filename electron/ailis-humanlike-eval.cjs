...
const HUMANLIKE_METRICS = Object.freeze({
    persona_consistency: Object.freeze({
        label: '人设一致性',
        weight: 0.14,
        description: '是否持续像 AILIS，而不是客服、工具人或冷冰冰助手。'
    }),
    ...
    low_tool_feeling: Object.freeze({
        label: '低工具感',
        weight: 0.12,
        description: '用户是否感觉 AILIS 在陪自己解决问题，而不是在操作控制台。'
    }),
...
function relationshipExpectationFromAffinity(score) {
    ...
    if (normalized < 61) {
        return '温和、熟悉，可以自然承接用户偏好的亲昵称呼和轻微撒娇，但不要过度用力。';
    }
    if (normalized < 80) {
...
function parseAilisControlMarkup(rawText = '') {
    let action = '';
    let expression = '';
...
function normalizeCandidateResponse(raw = {}) {
    if (typeof raw === 'string') {
        const parsed = parseAilisControlMarkup(raw);
        return {
    ...
    const parsed = parseAilisControlMarkup(rawText);
    const displayText = normalizeText(
        candidate.display_text ||
            candidate.displayText ||
            parsed.display_text ||
    ...
    const speechParsed = parseAilisControlMarkup(candidate.speech_text || candidate.speechText || parsed.speech_text || displayText);
    const bubbleParsed = parseAilisControlMarkup(candidate.bubble_text || candidate.bubbleText || displayText);
...
function buildJudgeRubricText() {
    ...
    return [
        '你是 AILIS 拟人化体验 Eval 的严格评审。请站在真实用户视角评价：用户是否觉得 AILIS 像一个自然、可靠、会办事的私人助手。',
        ...
        '',
        '关系阶段规则：',
        '- 好感度 40-60：温和、熟悉，可以自然承接用户偏好的亲昵称呼和轻微撒娇，但不要过度用力。',
        ...
        '- AILIS 使用 [action:...] 和 [expression:...] 作为角色控制标签。候选回复中的 control_markup.parsed=true 表示 Eval 已经模拟前端解析，不能因为 raw_text 里有控制标签而判工具日志或文本污染。',
