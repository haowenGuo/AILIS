...
function shouldRetryCodexBridgeFailure(result = {}) {
    ...
    if (code === 'codex_app_server_protocol_error') {
        const rpcMethod = normalizeText(result?.details?.rpcMethod).toLowerCase();
        const rpcCode = Number(result?.details?.rpcCode);
        const diagnostic = normalizeText(result?.error).toLowerCase();
        return rpcMethod === 'turn/start' &&
            rpcCode === -32603 &&
            diagnostic.includes('failed to start turn:') &&
            diagnostic.includes('internal error; agent loop died unexpectedly');
    }
