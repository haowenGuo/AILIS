...
function getAILISSkill(skillId) {
    const id = normalizeString(skillId);
...
function listAILISSkills() {
    return [...new Set([...Object.keys(DEFAULT_SKILLS), ...listSkillIdsFromDisk()])]
        .map((id) => getAILISSkill(id))
        .filter(Boolean);
}

function listAILISSkillSummaries() {
    return listAILISSkills().map((skill) => ({
        id: skill.id,
        label: skill.label,
...
function buildAILISSkillContextText(skillId, options = {}) {
    const skill = getAILISSkill(skillId);
    if (!skill) {
        return '';
    }
    const heading = [
