...
const { applyLocalPatch, parseLocalPatch, patchBodyToText, applyUpdatePatchText } = require('./ailis-local-patch.cjs');
...
class AILISGateway extends EventEmitter {
    ...
    parseLocalPatch(input = '') {
        return parseLocalPatch(input);
    }

    patchBodyToText(body = []) {
        return patchBodyToText(body);
    }

    applyUpdatePatchText(source = '', body = []) {
        return applyUpdatePatchText(source, body);
    }
