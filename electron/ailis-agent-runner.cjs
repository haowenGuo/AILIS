...
function buildLlmAgentDirectToolPrompt({
    ...
    const instructions = [
        ...
        'When a tool result says outputComplete=true, outputTruncatedForModel=false, complete=true, truncated=false, or reasoning_ready=true, treat it like a Codex source viewport: answer from it if it covers the question. Continue only when a concrete required field is still missing. Older exploratory observations may be compacted; rely on the latest complete evidence or write a focused verifier.',
