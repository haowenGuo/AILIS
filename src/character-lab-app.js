...
function getCharacterAdapterLabel(adapter) {
    return String(adapter || '').trim() || 'VRM 1.0';
}
...
function updateCharacterSelectionMeta() {
    ...
    if (!selected) {
        elements.characterPackageMeta.textContent = '当前渲染器没有提供可切换的人物包。';
        return;
    }
...
async function switchCharacter() {
    ...
    setStatus('正在切换人物资源，聊天和 Agent 不会中断。', '切换人物');
    try {
...
function applyCapabilityLabels() {
    if (!state.capabilities || state.capabilities.backend === 'electron') {
        return;
    }
...
async function applySurface(surface, label) {
    ...
    setStatus('命令已发送到 Three.js 后端', label);
    return true;
}
...
function getMotionCatalog(capabilities) {
    if (!Array.isArray(capabilities?.motions)) {
        return [];
    }
...
function renderAnimationState(snapshot) {
    ...
    if (!supported) {
        ...
        elements.animationDebugNote.textContent =
            '状态来自当前人物渲染器。暂停与定位只影响动画预览，不会改变聊天或 Agent 状态。';
    }
}
...
async function refreshAnimationState() {
    ...
    try {
        ...
        if (result?.animation) {
            ...
            elements.animationDebugNote.textContent =
                `状态机读取失败：${result?.error || '当前渲染器不支持动画状态调试'}`;
        }
    } catch (error) {
        elements.animationDebugNote.textContent =
...
function createExperienceSteps() {
    return [
        ...
        },
        {
            ...
            holdMs: 3200,
            run: async () => {
                const text = '你好呀，我们来检查 AILIS 的完整人物体验。';
...
async function refreshRuntimeStatus() {
    try {
        ...
        const backend = 'Three.js';
        const detail = runtime?.status === 'ready' ? '已连接' : runtime?.status || '未就绪';
