...
export function mapAudioEnvelopeToMouthValue(envelope, articulationPulse = 1) {
    ...
    const articulationMix = lerp(
        CONFIG.AUDIO_LIP_SYNC_SUSTAIN,
        1,
        clamp(Number(articulationPulse) || 0, 0, 1)
    );
    return clamp(
        shapedEnergy * CONFIG.AUDIO_LIP_SYNC_BOOST * articulationMix,
        0,
...
export class TTSAudioPlayer {
    ...
    updateLipSyncFromAudio() {
        ...
        const articulationPulse = Math.pow(
            0.5 - 0.5 * Math.cos(this.lipSyncPulsePhase * TWO_PI),
            CONFIG.AUDIO_LIP_SYNC_PULSE_SHAPE
        );

        this.vrmSystem.setLipSyncValue(
            mapAudioEnvelopeToMouthValue(this.lipSyncEnvelope, articulationPulse)
        );
    }
