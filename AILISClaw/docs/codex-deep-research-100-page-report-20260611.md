...
**结论**：这份报告的目标是解释 Codex 如何把复杂任务做稳，并把结论转成 AILIS 可施工的 100 页路线图。最终判断：AILIS 不应继续堆 prompt，而应重建 transcript-driven evidence executor。

**Codex 机制**：Codex 的优势不在单句提示，而在 runtime 把输入、工具、观察、权限、补丁、事件和下一轮决策做成闭环。模型负责推理，harness 负责把推理变成可恢复、可审计、可验证的行动。

**AILIS 差距**：AILIS 已经有 direct-tool、tool_search、recent_turn_items 和 finalizer 的雏形，但多处仍停在摘要、提示和事后补救层，尚未形成证据驱动的主循环。
...
**结论**：调研采用三层证据：官方 OpenAI 文档确认产品/API 语义，本地 openai-codex 源码确认实现形状，AILIS/GAIA 日志确认当前失败点。
...
**结论**：2026-06-11 的 24 题分类跑法显示，AILIS 只有视频/视觉类中 2 题通过，其余长链、网页证据、结构化来源和 finalizer 类均未得分。
...
**结论**：每页都是一张工程研究卡：先读结论，再看 Codex 机制、AILIS 差距、改造建议、验收点和证据。后续可以按页拆 issue。
...
**结论**：A 级来自源码或官方文档，B 级来自多处源码的强推论，C 级是给 AILIS 的工程建议。实现时先采纳 A+B，再评估 C。
...
**AILIS 差距**：AILIS 的主循环已能构造 directToolSpecs，但工具结果大量压缩进 recent_turn_items 和 lossless_tool_observations；这让模型可见的是摘要，而不是完整的、可引用的执行事件。
...
**AILIS 差距**：AILIS 已有 recent_turn_items 和 prompt_compaction 设计，但任务链仍可能把长日志、重复 progress、摘要 observation 多次塞回模型。
...
**证据**：turn; ailis; F:/AILIS/docs/ailis-codex-context-compaction.md

**证据展开**：turn: .refs/openai-codex/codex-rs/core/src/session/turn.rs:141,163,236,640,692,770,892,1098,1106,1616 | ailis: F:/AILIS/electron/humanclaw-agent-runner.cjs:3544,3545,3561,3562,5175,5197,5223,5484,5607,5678,5798；humanclaw-tool-contracts.cjs:538,584,661,682；humanclaw-runtime.cjs:471,528,771,923；humanclaw-gateway.cjs:663,717,734,885,1635,1695 | F:/AILIS/docs/ailis-codex-context-compaction.md: F:/AILIS/docs/ailis-codex-context-compaction.md
...
**AILIS 差距**：AILIS 有 tool_search contract 和 Gateway 搜索能力，但 GAIA 失败里仍出现 tool_search 参数用 question 而不是 query，说明模型没有始终在真实 schema 上生成调用。
...
**结论**：Codex 的 ToolSearchOutput 会进入模型可见输出；AILIS 应返回 tool id、schema、source、provenance 和 call pattern，而不是自然语言建议。
...
**结论**：MCP 不只是 function call，还包括 resources、templates 和上下文。AILIS 要避免只实现 tool bridge，却忽略资源读取和证据引用。
...
**结论**：Skill loader 读取 SKILL.md metadata、dependencies 和 policy；AILIS 的能力包也应能声明依赖工具缺失时的 degraded 状态。
...
**结论**：Codex 会把仓库局部指令纳入上下文；AILIS 也要支持项目级约束，但不能让它覆盖安全和 tool schema。
...
**结论**：Codex 把工具调用、非工具回复、事实和完成项作为可处理的 items；AILIS 应避免把所有历史压成一段自然语言状态。
...
**结论**：真正的续跑不是重新描述进度，而是把上一轮 response item 和工具结果继续送入下一轮。AILIS 目前摘要链路仍会丢细节。
...
**AILIS 差距**：AILIS 的 observation 目前仍过度依赖 preview/digest；ClinicalTrials 和 PPT 案例说明答案可能出现在过程里，但 finalizer 未拿到稳定证据槽。
...
**结论**：Codex 暴露 cmd、workdir、tty、yield_time_ms、max_output_tokens、权限参数等字段；这比 AILIS 的泛化 computer.exec 更适合模型稳定调用。
...
**AILIS 差距**：AILIS 有 computer.exec、session_start、pty_* 和 code.test 等多条执行路径，模型侧入口碎片化；长任务和脚本输出不总能稳定成为 answer-bearing evidence。
...
**结论**：Codex 计算 original_token_count，再按 max_output_tokens/truncation policy 返回模型可见文本；AILIS 的日志/摘要也应保留原始规模元数据。
...
**结论**：本项目在 Windows 上运行，Codex 源码有 Windows sandbox 分支；AILIS 的执行层也要把路径、权限、shell 语义作为平台差异处理。
...
**AILIS 差距**：AILIS 已有 apply_patch、request_permissions contract 和路径守卫，但普通任务仍可能把源码修改、脚本执行、权限阻断混在 meta JSON plan 里，缺少统一生命周期。
...
**证据**：official; patch_perm; ailis

**证据展开**：official: 官方资料：Codex CLI https://developers.openai.com/codex/cli；Codex MCP https://developers.openai.com/codex/mcp；Apply Patch https://developers.openai.com/api/docs/guides/tools-apply-patch；Tool Search https://developers.openai.com/api/docs/guides/tools-tool-search；Shell https://developers.openai.com/api/docs/guides/tools-shell；Web Search https://developers.openai.com/api/docs/guides/tools-web-search；Agents SDK https://developers.openai.com/api/docs/guides/agents | patch_perm: .refs/openai-codex/codex-rs/core/src/tools/handlers/apply_patch.rs:301,313,363,442,502,515；request_permissions.rs:32,55,64,78；permissions_instructions.rs:62 | ailis: F:/AILIS/electron/humanclaw-agent-runner.cjs:3544,3545,3561,3562,5175,5197,5223,5484,5607,5678,5798；humanclaw-tool-contracts.cjs:538,584,661,682；humanclaw-runtime.cjs:471,528,771,923；humanclaw-gateway.cjs:663,717,734,885,1635,1695
...
**结论**：Codex apply_patch handler 会先验证参数、路径和 correctness error，再决定应用或返回错误。AILIS 应把失败暴露给模型修复。
...
**结论**：安全边界包括 filesystem、network、shell、destructive command 和 prefix rule。AILIS 需要统一策略，不要让各工具各自判断。
...
**结论**：Codex 的 exec policy 区分 safe command、危险操作和审批路径；AILIS 至少要对删除、移动、网络、workspace 外写入做硬规则。
...
**结论**：权限申请的价值在于 grant 后继续原任务，而不是把当前轮中断成用户手工操作。AILIS 需要 grant store 和 retry continuation。
...
**结论**：Codex 对函数输出和 exec 输出使用不同 truncation policy，并保留 omission/原始规模提示。AILIS 也要区分 UI trace 与 model context。
...
**结论**：Codex 跟踪 auto_compact_window_ordinal 和 prefill tokens；AILIS 的长任务也应记录当前窗口和压缩前后 token 量。
...
**结论**：Codex 有 inline/remote compaction 路径；AILIS 不必照搬服务，但必须有本地可恢复检查点，尤其在远程 compact 失败时。
...
**结论**：官方 Web Search 语义是让模型获取最新信息；对 GAIA/AILIS 来说，还要把 search result、fetch、page locator 和 evidence artifact 分层。

**Codex 机制**：网页证据定位需要把页面当结构化对象处理：URL、DOM/文本、渲染、字段、附近上下文、格式差异都可能是证据，不应只靠搜索摘要。

**AILIS 差距**：网页证据定位 2/2 失败，说明页面已找到不等于字段已定位；HTTP 403、格式化诗行、Word-of-the-Day 引文作者都需要页面内控制器。
...
**证据展开**：official: 官方资料：Codex CLI https://developers.openai.com/codex/cli；Codex MCP https://developers.openai.com/codex/mcp；Apply Patch https://developers.openai.com/api/docs/guides/tools-apply-patch；Tool Search https://developers.openai.com/api/docs/guides/tools-tool-search；Shell https://developers.openai.com/api/docs/guides/tools-shell；Web Search https://developers.openai.com/api/docs/guides/tools-web-search；Agents SDK https://developers.openai.com/api/docs/guides/agents | gaia: F:/AILIS/scripts/run-gaia-level1-lite.mjs:328,355,414,425,600,622；F:/AILIS/eval-results/engineering/gaia-official/*20260611*.summary.json；F:/AILIS/docs/ailis-vs-codex-gaia-failure-source-report-20260611.md
...
**结论**：EvidenceArtifact 是 AILIS 追上 Codex 稳定性的关键抽象：把 answer-bearing field 从工具摘要里解放出来。
...
**AILIS 差距**：视频/视觉 4 题只过 2 题：可被网页/字幕救的通过，鸟类同屏计数和棋盘结构化失败，说明缺 frame-to-structure 链路。
...
**证据展开**：official: 官方资料：Codex CLI https://developers.openai.com/codex/cli；Codex MCP https://developers.openai.com/codex/mcp；Apply Patch https://developers.openai.com/api/docs/guides/tools-apply-patch；Tool Search https://developers.openai.com/api/docs/guides/tools-tool-search；Shell https://developers.openai.com/api/docs/guides/tools-shell；Web Search https://developers.openai.com/api/docs/guides/tools-web-search；Agents SDK https://developers.openai.com/api/docs/guides/agents | exec: .refs/openai-codex/codex-rs/core/src/tools/handlers/shell_spec.rs:20,103,217,229,267；unified_exec/exec_command.rs:72,222,236,279,291；write_stdin.rs:32,47,100；process_manager.rs:432,443,578,595,643,668,1093,1219 | gaia: F:/AILIS/scripts/run-gaia-level1-lite.mjs:328,355,414,425,600,622；F:/AILIS/eval-results/engineering/gaia-official/*20260611*.summary.json；F:/AILIS/docs/ailis-vs-codex-gaia-failure-source-report-20260611.md
...
**AILIS 差距**：长链 6 题 0/6，典型症状是反复 web_search、没有稳定 canonical source、没有把上一跳证据作为下一跳输入。
...
**结论**：9d191bce 和 0383a3ee 能过，说明网页/字幕证据可救一部分视频题；AILIS 应把 transcript acquisition 做成标准路径。
...
**结论**：8 道结构化来源题全败，说明 AILIS 的核心缺口不是答案推理，而是稳定进入正确数据源和字段路径。

**Codex 机制**：专用数据题的正确路径通常是 adapter 或稳定表格，而不是泛搜。Codex 的优势是能更早切换到 API、下载表格、写脚本、读取 JSON path。

**AILIS 差距**：结构化来源 8/8 失败，ClinicalTrials 甚至调用了 external tool 仍提交空答案，说明 adapter correctness 和结果传递链都不稳。
...
**AILIS 差距**：finalizer/格式化 4/4 不得分：有空提交、整段中文解释提交、低置信错误提交和 b,e 格式问题。
...
**证据展开**：gaia: F:/AILIS/scripts/run-gaia-level1-lite.mjs:328,355,414,425,600,622；F:/AILIS/eval-results/engineering/gaia-official/*20260611*.summary.json；F:/AILIS/docs/ailis-vs-codex-gaia-failure-source-report-20260611.md | ailis: F:/AILIS/electron/humanclaw-agent-runner.cjs:3544,3545,3561,3562,5175,5197,5223,5484,5607,5678,5798；humanclaw-tool-contracts.cjs:538,584,661,682；humanclaw-runtime.cjs:471,528,771,923；humanclaw-gateway.cjs:663,717,734,885,1635,1695
...
**结论**：AILIS direct mode 已开始暴露 native direct tools，但 fallback/meta JSON 仍会让 nested args 脆弱，尤其是 MCP/external 工具。
...
**结论**：AILIS contract 明确要求 query/q，但模型仍错参；修复方向是让 tool_search 成为原生工具，并用 schema validation 生成可修复 observation。
...
**AILIS 差距**：AILIS 当前最危险的不是没有工具，而是多条半成品链路并存：meta planner、direct tools、summary observation、finalizer、adapter 各自工作但没有闭环。
...
**证据**：gaia; F:/AILIS/AILISClaw/docs/codex-code-architecture-guide.md; F:/AILIS/AILISClaw/docs/ailis-codex-mainline-spec.md

**证据展开**：gaia: F:/AILIS/scripts/run-gaia-level1-lite.mjs:328,355,414,425,600,622；F:/AILIS/eval-results/engineering/gaia-official/*20260611*.summary.json；F:/AILIS/docs/ailis-vs-codex-gaia-failure-source-report-20260611.md | F:/AILIS/AILISClaw/docs/codex-code-architecture-guide.md: F:/AILIS/AILISClaw/docs/codex-code-architecture-guide.md | F:/AILIS/AILISClaw/docs/ailis-codex-mainline-spec.md: F:/AILIS/AILISClaw/docs/ailis-codex-mainline-spec.md
...
**结论**：AILIS 需要每轮记录 prompt tokens、tool output tokens、omitted items、active window 和 artifact refs；70% 触发检查点。
